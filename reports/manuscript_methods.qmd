---
title: "Worst-Case Selection of Targeted-Learning Estimators under Prespecified Data-Quality Threats: An Outcome-Blind Workflow"
author:
  - name: "Andrew Mertens"
...
Estimator selection in real-world evidence usually ranks candidate
estimators by their error on the data as observed, which ignores how each
candidate behaves when the data fall short of the assumptions the analysis
relies on. We develop an outcome-blind procedure that selects among
prespecified targeted-learning estimators by their worst-case performance
across a declared set of data-quality threats, and we show that this
worst-case rule can lock a different estimator from the rule that minimises
error on undisturbed data. In a constructed demonstration with three
propensity-score-truncation candidates and a prespecified threat sweep that
includes a positivity violation as a first-class package threat, the baseline
rule (`min_rmse`) selected the heavily truncated candidate and the worst-case
rule (`min_max_rmse`) selected an intermediate candidate, with the two winners
separated on worst-case root mean squared error by more than six times the
combined Monte Carlo standard error. The demonstration establishes that such
divergence occurs under realistic threats, and it does not claim the divergence
is generic.

The procedure sits inside a staged, outcome-blind targeted-learning workflow
for binary point-treatment studies, implemented in the R package cleanTMLE.
The analyst defines the causal question, the observed data, the identification
assumptions, the statistical estimand, and a set of candidate estimators, then
compares the candidates on synthetic outcomes before inspecting the observed
treatment-outcome association. The data-quality stress test extends this loop
by scoring each candidate under prespecified threats on the same decision scale
as the baseline plasmode. A fingerprinted analysis lock, an audit trail, and a
decision log record the prespecification so that an external reviewer can
reconstruct it.
...
treatment structure and evaluates each candidate under prespecified
threats, including covariate missingness, exposure misclassification,
...
(95% CI coverage `r round(mt_A_cov_if, 2)`, Monte Carlo SE
`r round(mt_A_cov_if_se, 2)`), because it ignores
matching-draw variance, the variability from which controls are
selected when data are re-sampled. The standard nonparametric
bootstrap is not consistent for matching estimators (Abadie and
Imbens 2008); a full-pipeline variant that re-runs matching on each
resample raised coverage to `r round(mt_A_cov_boot, 2)` in this
comparison, which we report as an empirical finding. The stabilised
IPTW IF-SE is moderately conservative under marginal overlap (SE/SD
ratio ≈ `r round(iptw_B_ratio_if, 2)`) because it treats the
propensity score as known, and correcting this requires an
M-estimator rather than resampling. `select_variance_method()` uses oracle coverage on
...
largest degradation in estimator performance. In a constructed
demonstration with a propensity-score-truncation candidate grid and a
prespecified threat sweep that includes a positivity violation, the
worst-case selection rule (`min_max_rmse`) selected a different
candidate from the baseline rule (`min_rmse`), which shows that
screening candidates against their worst case across data-quality
threats can change which estimator is locked.
...
test, which extends that selector to score each candidate against
prespecified data-quality threats on the same bias, RMSE, and coverage
scale (see @sec-dq for the full DQ framework; @sec-divergence for
evidence that the worst-case selection rule can change the locked
estimator).

Around these statistical components the package adds an optional
transparency layer: an analysis-lock object, a serialisable audit trail
and decision log in the sense of @muntner2024staging, and a pre-outcome
decision rule that applies the three review-team recommendations (GO,
FLAG, STOP) by withholding primary-analysis functions until the
prespecified checkpoints are recorded. This software arrangement sits
inside, rather than substitutes for, the personnel-based governance a
clean room requires, such as role separation and independent review
(@sec-discussion). @sec-roadmap situates the package inside the causal
roadmap.

The scope of this paper is binary point-treatment studies with a
marginal risk-difference estimand; limitations on estimand coverage are
listed in @sec-discussion. This paper documents version 0.1.5 of the
package; case-study estimates in @sec-casestudy were produced under
0.1.1 and are numerically unchanged under the backward-compatible
0.1.5 API.

## What the paper contributes

The contribution is a single, prespecified, outcome-blind procedure that
ties qualitative fit-for-purpose threats to quantitative estimator
selection on one decision scale, together with a reproducible software
implementation. We extend the outcome-blind plasmode loop
...
prespecified threat ranges, and we characterise the screen's operating
characteristics as a decision rule (@sec-gate-oc).

FIORD [@nance2026fiord; @kent2026fiord] supplies the outcome-blind
candidate-evaluation loop and the two-stage selector. This paper
contributes the software implementation of that loop, the data-quality
stress-test extension, and the lock, audit, and pre-outcome-decision
record. We hold to this division of labour throughout.

The screen is a quantitative supplement to fit-for-purpose review
(@sec-bridge); what passing it does and does not establish is stated in
@sec-guarantees. The implementation records the SHA-256-fingerprinted
analysis lock, the audit trail, the decision log, and the pre-outcome
decision, so the screen and the locked primary analysis share a single
reproducible record.
...
not run, whereas the conventional pipeline reports a biased non-null
estimate (mean bias 0.007) that would be published. The fits are
identical under both pipelines; the difference is the prespecified
decision applied before the outcome is read.
...
plug-in or single-model estimator does. The candidate estimators are
...
The manuscript uses a fixed vocabulary for the conceptual layers of the
workflow (defined in @tbl-layers, Appendix). @fig-roadmap places the
package inside the roadmap and @fig-governance situates it within the
broader governance stack. @tbl-reg-align maps each roadmap component to
regulatory expectations and to what still requires external
study-level work; @tbl-event-diag lists the event-process helpers.

The remaining sections follow the roadmap order: @sec-estimand states
the estimand and identification assumptions; @sec-framework describes the
lock, audit, and pre-outcome-decision functions; @sec-dq introduces the
DQ stress test; @sec-bridge frames it as a supplement to fit-for-purpose
review; @sec-implementation--@sec-casestudy report the implementation,
simulation, and case study; and @sec-discussion covers limitations.
...
software-mediated workflow restricts outcome analysis until
prespecified review steps have been completed. It supplements
personnel-based governance rather than substituting for it. The audit
and decision logs give reviewers transparent documentation of every
analytic decision alongside the institutional record that already
exists.
...
realised data. A defensible RWE pipeline can apply the
simulation-based workflow inside an institutional partition of duties.

### Personnel-based and simulation-mediated protections compared
...
follows the guidance of @nance2026fiord. cleanTMLE's default plasmode
uses the empirical $(W, A)$ structure with a covariate-only outcome
model, never the joint $(A, Y) \mid W$ relationship, placing it in the
hybrid middle of the FIORD data-generating-process spectrum. The DQ
stress test extends this by adding a prespecified perturbation step on
top of the same hybrid generator.
...
The precise scope of what a passing screen establishes, and what remains
an external study-level argument, is stated in @sec-guarantees.
...
The outcome-blind property of the plasmode, defined in the introduction
(the marginal $E[Y \mid W]$ is fitted on the realised outcome and the
joint treatment-outcome association is never inspected before the
pre-outcome decision), carries through to the DQ stress test. The
analysis-lock fingerprint records the final choices; once all pre-outcome
stages are complete, the outcome is unmasked and the primary analysis runs.

## What passing the screen does and does not establish {#sec-guarantees}

Passing the DQ stress test establishes only that, under the analyst's
synthetic-outcome model and over the prespecified range of each threat,
the locked candidate maintained bias, coverage, and SE calibration within
the prespecified thresholds. It does not establish that the identification
assumptions hold, that the data are fit for purpose, or that the realised
estimator will behave as the synthetic one did. Three caveats follow.
First, the screen can only score threats the analyst names: a confounder,
misclassification pattern, or missingness mechanism that is not
anticipated, or specified at too mild a severity, is not detected, so a
STOP is evidence of fragility but a GO is not evidence of robustness.
Second, the conclusions are conditional on the synthetic-outcome model;
if that model misrepresents the covariate-outcome surface the simulated
operating characteristics can be optimistic, which is why we report the
screen's calibration under a misspecified model in @sec-gate-oc and
recommend a SuperLearner library for the synthetic-outcome model.
Third, the screen is a pre-outcome device; it does not replace the
at-outcome negative-control check or the post-outcome sensitivity analyses
that interrogate the realised estimate (see @sec-evalue for the E-value
comparison). A passing screen is a necessary, prespecified, and
reproducible filter, not a certificate of validity.
...
does not reveal.
@tbl-boot-variance reports a targeted 100-replicate variance comparison for
IPTW and Match_TMLE across three overlap regimes. Overlap regime C has minimal
PS spread (very good overlap); overlap regime A has moderate PS spread (good
overlap); overlap regime B has wide PS spread (marginal overlap). These regimes
are distinct from the main simulation scenarios A, B, and C described in
@sec-simulation. For each replicate both the IF-based and nonparametric
bootstrap (B = 200 draws, GLM PS) SE are computed; the table reports mean
SE/SD ratios and 95% CI coverage under each method.

Under very good overlap (regime C), both IPTW and Match_TMLE are approximately
calibrated (`r round(mt_C_cov_if, 2)` IF-coverage for Match_TMLE, Monte Carlo
SE `r round(mt_C_cov_if_se, 2)`), and the bootstrap shows no material
difference at this sample size. When PS spread is minimal the matching-draw
variance is smaller relative to the outcome variance, so the IF-SE is already
at nominal coverage. Oracle selection would find no difference to act on here;
100 replicates can fail to detect a small difference, so this comparison does
not establish equivalence.

Under good overlap (regime A), IPTW achieves se_sd_ratio_if ≈
...
#|   three overlap regimes (C: very good, A: good, B: marginal),
#|   100 MC replicates per regime, B = 200 bootstrap draws.
#|   se_sd_ratio is the mean SE divided by the empirical SD of the
...
#|   Monte Carlo standard error in parentheses; at 100 replicates this
...
disp <- data.frame(
  ...
  check.names = FALSE, stringsAsFactors = FALSE)
knitr::kable(disp, digits = c(NA, NA, 4, 3, NA, 3, NA),
             caption = NULL)
```
...
```{r}
#| label: fig-mc-boxplots
#| eval: !expr has_mc
#| fig-height: 9
#| fig-cap: "Sampling distribution of the risk-difference estimate across Monte Carlo replicates. The dashed red line marks the true RD. Under unmeasured confounding (C), all estimators are shifted from the truth."
...
```{r}
#| label: fig-coverage-bar
#| eval: !expr has_mc
#| fig-cap: "95% CI coverage by estimator and scenario. The dashed red line marks the nominal 95%."
...
#| eval: !expr has_mc
#| fig-cap: "Ratio of mean reported SE to empirical SD. Values near 1.0 indicate well-calibrated standard errors."
...
```{r}
#| label: fig-dq-degradation
#| eval: !expr has_dq
#| fig-height: 10
#| fig-cap: "Degradation gradient. Bias (top) and coverage (bottom) for the selected TMLE candidate at each severity level of each DQ scenario, faceted by Monte Carlo scenario."
...
RMSE across scenarios, so both selectors choose the same candidate
in every cell. This grid is too narrow to exercise the contrast
between the two rules. The FIORD coverage screen remains informative
as a diagnostic. Under marginal overlap (Scenario B) neither
candidate achieves oracle coverage within the 0.02 tolerance of
nominal, which the table flags as "no candidate within tol", a
property of the near-positivity DGP at this replicate count. The
contrast between `min_rmse` and `min_max_rmse` becomes substantive
when the candidate grid separates the candidates, which the
constructed demonstration in @sec-divergence exhibits.

# Worst-case selection changes the locked estimator {#sec-divergence}

The Monte Carlo study in @sec-simulation validates estimator behaviour. This
section presents the central result. The grid of three truncation candidates
does separate on baseline RMSE, and the `min_max_rmse` rule selects a different
candidate from `min_rmse`. The constructed demonstration below confirms this and
exhibits the mechanism. The demonstration's purpose is existence: it
shows that realistic threat configurations exist under which the two rules
diverge. The severities were chosen to exhibit the mechanism, so the
demonstration establishes that divergence is possible rather than that it is
generic. The whole sweep runs through the package functions
`run_plasmode_feasibility()`, `run_plasmode_dq_stress()` (with
`near_positivity` as a first-class threat), and `select_tmle_candidate()`, on
the same `run_simulation.R` data-generating process. The driver and its results
are in the companion repository (`sandbox/candidate_divergence/`).
...
thresholds were locked before the sweep, allowing at most a doubling of root
...
about the causal effect of Rescue.Co transport. The registry compares
prehospital transport by a specialised emergency-medical-services
provider (Rescue.Co) with traditional ambulance care at three Nairobi
hospitals, on functional outcome six months post-injury. All five stages
were run on cleanTMLE 0.1.1 and the example is fully reproducible from
the bundled `rescueCo/` directory in the companion repository. The DQ
stress test uses the 200-replicate run (see @sec-casestudy-dq).
...
| 2026-01-12 11:02 | Stage 2c | DQ stress reported at `n_sims = 200` | `n_sims` >= 200 for inferential interpretation | GO | Report the 200-replicate run; the bundled `n_sims = 3` configuration is a CI smoke test only | Lead analyst | No | Locked `glm_t01` as minimax winner | No |
...
0.21--0.81). This grid does not separate the candidates, as in the
main simulation. @sec-divergence reports a constructed grid that
does separate them, where the two selection rules choose different
candidates.
...
over the candidate grid, and the minimax rule locks `glm_t01` as the
primary specification. The reported result uses the 200-replicate run
(`inst/extdata/rescueco_dq_full.R`, see @sec-reproducibility). The
3-replicate configuration bundled with the package is a
...
structural pass on each scenario at 200 replicates per threat-severity
cell (see @sec-reproducibility for the entry point). The four
...
The package's contribution is to make staged prespecification and the
pre-outcome decision rule explicit in code and reproducible by an external
reviewer. A precise test of scope is to ask what evidence the package can and
...
The lock, audit, and decision-log records track the prespecification and
traceability that current real-world-evidence guidance expects, as mapped in
@tbl-reg-align. The software layer sits inside the personnel-based governance
this guidance requires, and does not substitute for it.
...
threats) but cruder (it does not adjust the estimate). The two
tools address different questions and both should be reported.
...
The simulation results reported in this paper are produced by three
scripts in the companion repository
(<https://github.com/amertens/clean-room-sim>). Each script writes its
output to the `results/` directory, which is tracked in the repository
for the files listed below.

| Script | Output file(s) | Figures / tables |
|---|---|---|
| `run_simulation.R` | `results/simulation_results.rds` | @tbl-mc-metrics, @fig-mc-boxplots, @fig-coverage-bar, @fig-se-calibration, @fig-mc-forest, @fig-dq-degradation |
| `run_gate_operating_characteristics.R` | `results/gate_oc.rds` | @sec-gate-oc figures |
| `_bootstrap_variance.R` | `results/bootstrap_variance.rds`, `results/bootstrap_variance.csv` | @tbl-boot-variance |
| `inst/extdata/rescueco_dq_full.R` | `results/rescueco_dq_full.rds` | Case-study DQ subsection (@sec-casestudy-dq) |
...
200-replicate run produced by `inst/extdata/rescueco_dq_full.R`. The
