...
for IPTW and Match_TMLE across three overlap regimes. Overlap regime I
has minimal PS spread (analogous to very good overlap); overlap regime II
has moderate PS spread (good overlap); overlap regime III has wide PS
spread (marginal overlap). These regimes are distinct from the main
simulation scenarios A, B, and C described in @sec-simulation.
...
Overlap regime I (very good overlap) shows the strongest anti-conservatism for
...
Overlap regime II (good overlap) shows the same pattern, less severely.
...
Overlap regime III (marginal overlap) reverses the pattern for IPTW. The
IF-based variance treats the propensity score as known rather than
...
`r round(iptw_B_cov_if, 2)`). The bootstrap does not correct this
conservatism: both bootstrap and IF coverage remain near
`r round(iptw_B_cov_if, 2)`. The reason is structural. PS-estimation
...
underlying the IF-SE is approximately satisfied. For Match_TMLE in overlap
regime III and for IPTW across all three regimes, the bootstrap provides no
...
#|   three overlap regimes (I: very good, II: good, III: marginal),
#|   100 MC replicates per regime, B = 500 bootstrap draws.
...
evidence in overlap regime II (IF-coverage 0.89, bootstrap-coverage 0.95)
validates this theoretical recommendation empirically. In overlap regime III,
where the caliper restricts the matched cohort to a well-overlapped
sub-population, the matching-draw variance is less severe and the IF-SE
happens to be approximately calibrated; oracle selection would correctly
identify this as a setting where the two methods are equivalent. For IPTW, neither variance
...
primary specification. The bundled configuration runs 3 replicates
per threat-severity cell, which is sufficient for a reproducibility
check but not for inferential interpretation; the companion script
`inst/extdata/rescueco_dq_full.R` reruns the same sweep at 200
replicates to confirm the minimax winner (see @sec-reproducibility).
...
a staged pre-outcome classification of the analysis before
...
: Script-to-output map. Running all four scripts in order fully
...
The bundled configuration (`config/clean_room_config.yml`) runs
Stage 2c at 3 replicates per threat-severity cell for fast
reproducibility verification. The full 200-replicate run for the case
study DQ results uses `inst/extdata/rescueco_dq_full.R`.
