...
usually developed in parallel but reported separately: estimator
specification is supported with outcome-blind simulation, whereas
data fitness is documented qualitatively through target-trial
articulation, reporting templates such as TARGET and START-RWE,
and fit-for-purpose checklists such as SPIFD2 and HARPER.

We present **cleanTMLE**, an R package that provides a
prespecified quantitative stress test of estimator performance
under data-quality degradation scenarios derived from a prior
fit-for-purpose assessment, situated inside the causal roadmap.
The package does not itself determine whether a data source is
fit for purpose; that judgement remains a study-level activity
supported by target-trial protocols, validation studies, and
source-data review. Once investigators have specified plausible
data-quality threats and severity ranges from external evidence,
cleanTMLE records the prespecified analytic choices and audit
trail and extends the standard outcome-blind plasmode
estimator-selection loop with a pre-outcome stress test that
evaluates candidate estimators under those user-specified threats
(covariate missingness, exposure misclassification, outcome
misclassification, and unmeasured confounding). We illustrate the
workflow in a Monte Carlo simulation and in a trauma-registry case
study (n = 1,693 ambulance patients, 24.6 % outcome missingness)
where six estimators implemented in cleanTMLE following the
staged-analysis vignette agree on direction (risk difference 0.026
to 0.037 in favour of Rescue.Co transport, with the prespecified
primary full-cohort TMLE interval just crossing zero). The case
study is presented as a workflow demonstration on a real registry
rather than as evidence that the framework establishes causal
validity.
...
appear in the regulatory-alignment table and in the discussion.
The IPW, g-computation, and AIPW cumulative-risk estimators
provided by the proprietary `causalRisk` package are an
alternative implementation of related ideas, with which cleanTMLE
shares conceptual structure but does not share code.
...
outcome analysis. Third, we acknowledge `causalRisk` [@brookhart2024causalrisk] as a
proprietary alternative implementation of related cumulative-risk
estimators with which cleanTMLE shares conceptual structure but
not code. The next
...
knitr::kable(case_eff, digits = 4,
             caption = "rescueCo binary-outcome estimates (good functional outcome at 6 months, GOSE > 4) with the target population for each estimator. All six estimates produced by cleanTMLE 0.1.1 following the staged-analysis vignette.")
```
