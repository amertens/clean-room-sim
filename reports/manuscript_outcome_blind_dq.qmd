...
≈ 2.1, 95% CI coverage ≈ 1.00), while the nonparametric bootstrap
...
if (has_boot) boot_var <- read.csv(boot_path)
```
...
= 2.11, coverage = 0.995. The post-match TMLE step treats the
...
Scenario A the inflation is mild (se_sd_ratio ≈ 1.07) because good
...
≈ 1.0 and coverage_boot ≈ 0.95 in both scenarios. For IPTW, the
