...
under marginal overlap. A supplementary variance comparison showed
that the influence-function standard error for matched-cohort TMLE
substantially over-covers under marginal overlap (mean SE/SD ratio
≈ `r round(mt_B_ratio_if, 1)`, 95% CI coverage ≈ `r round(mt_B_cov_if, 2)`), while the nonparametric bootstrap
restored near-nominal coverage, motivating the package's
`select_variance_method()` stage. When
...
SE-to-empirical-SD ratio for TMLE and TMLE_CF stays near one across
all three scenarios because the cross-fitted influence-function
variance is approximately correct for doubly robust estimators on
i.i.d. data. Two estimators diverge from this pattern in ways that
matter for inference.

First, the IF-based SE for stabilised IPTW treats the propensity
score as known rather than estimated, making it theoretically
conservative [@robins1992rotnitzky]. In our DGP the conservatism
is modest (se_sd_ratio ≈ 1.00 in all scenarios), meaning IF-based
coverage is already near nominal and bootstrap variance provides no
practical gain. This is likely because the GLM propensity-score
model is nearly correctly specified on the simple logistic DGP; the
conservatism is expected to be larger when the PS model is more
flexible or more variable.

Second, and more consequentially, the matched-cohort TMLE (Match_TMLE)
shows severe IF-based variance inflation in Scenario B: se_sd_ratio
= `r round(mt_B_ratio_if, 2)`, coverage = `r round(mt_B_cov_if, 3)`. The post-match TMLE step treats the
matched cohort as an i.i.d. sample, ignoring the residual
variability introduced by the matching draw. This is not a
pathology of the DGP; it is a structural consequence of running
a standard influence-curve variance on a non-i.i.d. sample. In
Scenario A the inflation is mild (se_sd_ratio ≈ `r round(mt_A_ratio_if, 2)`) because good
overlap makes matching stable; in Scenario B, where the PS range
extends to [0.04, 0.99], the matching draw is substantially
variable and the IF variance overstates precision by a factor of
two.

@tbl-boot-variance reports a targeted 100-replicate variance
comparison for IPTW and Match_TMLE in Scenarios A and B. For each
replicate, both the IF-based and nonparametric bootstrap (B = 200
draws, GLM PS) SE are computed; the table reports mean SE/SD ratios
and 95% CI coverage under each method.
...
The bootstrap restores calibration for Match_TMLE: se_sd_ratio_boot
≈ `r round(mt_B_ratio_boot, 2)` and coverage_boot ≈ `r round(mt_B_cov_boot, 2)` in both scenarios. For IPTW, the
IF and bootstrap variances agree (both near 1.0), confirming that
the theoretical conservatism is small in this DGP. The practical
implication is that analysts using Match_TMLE as the prespecified
primary estimator — as in the Rescue.Co case study — should specify
bootstrap variance in the SAP rather than relying on the default
IF-based SE. cleanTMLE provides `bootstrap_rd_variance()` for this
purpose and `select_variance_method()` to choose between IF and
bootstrap by oracle coverage on synthetic data.
...
```{r gate-oc, eval = file.exists("../results/gate_oc.rds")}
#| label: tbl-gate-oc
oc <- readRDS("../results/gate_oc.rds")
knitr::kable(
...
```{r gate-oc-summary, eval = file.exists("../results/gate_oc.rds"), results = "asis"}
res <- readRDS("../results/gate_oc.rds")$results
