...
We present cleanTMLE, an R package that operationalises a staged,
outcome-blind targeted-learning workflow for binary point-treatment
studies. The package is situated inside the causal roadmap:
investigators first define the causal question, observed data,
identification assumptions, statistical estimand, and candidate
estimators, then use outcome-blind simulations to compare candidate
estimators before examining the observed treatment-outcome
association. cleanTMLE extends this workflow with a plasmode
data-quality stress test. Using the empirical covariate and
treatment structure, the package generates synthetic outcomes and
evaluates candidate estimators under prespecified threats,
...
We also demonstrate the workflow in the Rescue.Co Kenya Trauma
Registry (n = 1,693; 24.6 % outcome missingness). Six estimators
agreed on direction, with risk differences from 0.026 to 0.037 in
favour of Rescue.Co transport, while the prespecified primary TMLE
interval crossed zero. This case study is a workflow demonstration,
not a causal claim. cleanTMLE does not replace target-trial design,
fit-for-purpose data review, validation studies, independent
governance, or post-outcome sensitivity analyses. Its contribution
is narrower: it turns prespecified targeted-learning decisions,
outcome-blind estimator selection, and data-quality stress testing
into reproducible, auditable software artifacts.
...
cleanTMLE is an R [@rcoreteam2024] package that operationalises an
outcome-blind staged workflow for the estimator-selection and
stress-testing steps of the causal roadmap [@dang2023roadmap;
...
decisions, and final authorization state. It does not implement
personnel separation, source-data access controls, protocol
adjudication, phenotype validation, or regulatory review.
...
independent checkpoint review. §\ref{sec-roadmap} situates
the package inside the causal roadmap and Table
\ref{tbl-reg-align} maps each component to the corresponding FDA,
...
[@lmtppackage2021] until those features are added. The proprietary
`causalRisk` package [@brookhart2024causalrisk] is an alternative
implementation of related cumulative-risk estimators.
:::
...
supplement to fit-for-purpose review (§\ref{sec-bridge}) rather
...
single reproducible record. We acknowledge `causalRisk`
[@brookhart2024causalrisk] as a proprietary alternative
implementation of related cumulative-risk estimators with which
cleanTMLE shares conceptual structure but not code. The next
...
A complementary supplementary mapping (Table \ref{tbl-event-diag})
...
audit, and pre-outcome-decision functions. §\ref{sec-dq} introduces the DQ stress
...
- Propensity-score diagnostics assess overlap, balance, and design
  feasibility, not estimator operating characteristics.
...
Propensity-score diagnostics assess empirical comparability,
overlap, and weight stability, but they do not estimate the
operating characteristics of a candidate estimator.
Negative-control analyses can help detect residual bias for
selected control outcomes, but they do not directly evaluate how
the primary estimator behaves under prespecified measurement
error, missingness, or unmeasured-confounding scenarios. DQ
stress testing addresses a different question: whether the locked
estimator-selection decision is stable under fit-for-purpose
threats translated into quantitative perturbations before the
primary treatment-outcome association is inspected.
...
generalize to the realized outcome process. DQ stress testing
...
behavior under prespecified perturbations.
...
moves bias or coverage beyond the threshold. The candidate-selection rule
...
The proprietary `causalRisk` package [@brookhart2024causalrisk]
implements IPW, g-computation, and AIPW cumulative-risk estimators
with a cohort-specification grammar oriented toward staged,
prespecified observational analyses. Although cleanTMLE was
informed by the conceptual structure of that package, the two
packages do not share code, and cleanTMLE's estimation, gating,
audit, and DQ stress-test functions are independent
implementations. We acknowledge `causalRisk` as the closest
alternative implementation in the R ecosystem, while noting that
its proprietary licence means independent reproduction of any
specific function is up to the user.
...
unacceptable positivity or weight instability. When competing
...
Although hazard ratios remain common in time-to-event analyses,
they are not generally the most transparent primary summary for
regulatory RWE questions framed as target trials. Cumulative
risks, risk differences, and risk ratios at clinically meaningful
follow-up times more directly correspond to treatment-specific
counterfactual risks and are often easier to align with estimand
language. Hazard ratios may be reported as secondary analyses when
proportional hazards is plausible and the estimand is explicitly
defined on the hazard scale. The package's
`clean_risk_report_table()` produces a single structured table of
cumulative risks, contrasts, event counts, and the estimator that
produced each row.
...
workflow: confirming acceptable finite-sample behavior, detecting
instability under marginal overlap, and stopping when an
identification failure is built into the data-generating process.
...
Monte Carlo noise. Three patterns are robust to noise.
...
The matching-with-TMLE composition shows a similar pattern in
Scenario B, where the SE-to-empirical-SD ratio reaches roughly
two and coverage exceeds nominal. The conservativeness comes from
treating the matched cohort as if it were a fresh independent
sample at the TMLE step, ignoring the residual variability
introduced by the matching draw. Users reporting matched-TMLE as
primary should either bootstrap over the matching step or use a
...
§\ref{sec-limitations}. Treatment misclassification attenuates the
...
validity. This case study demonstrates auditability and workflow
behavior; it is not intended as evidence that Rescue.Co transport
has a causal effect. The simulation in §\ref{sec-simulation} is small (5
...
`config/clean_room_config.yml` sets `simulation.n_sims = 3`. At
this replicate count the per-cell coverage estimates are noisy
and the qualitative degradation pattern, not the absolute coverage
number, is what the decision rule consumes. The companion
configuration `inst/extdata/rescueco_dq_full.yml` runs the same
sweep at `n_sims = 200` and reproduces the same minimax winner.]
...
negative-control endpoints (chronic-pain history, prior fracture,
and elective surgery) were dropped from the NC analysis at the
data-sanitisation step because they failed near-zero-variance
filtering on the locked cohort; only chronic hypertension and
household urban status carried enough variation to fit a stable
PS-adjusted model. The audit log records the three drops alongside
...
case_eff <- data.frame(
  ...
  `Target population` = c(
    ...
    "Full eligible cohort",
    "Matched subset (overlap)",
    "Full eligible cohort under censoring model"),
...
sensitivity row. This case study demonstrates auditability and
workflow behavior; it is not intended as evidence that Rescue.Co
transport has a causal effect.
...
over the four threats and produces a structural pass on each
scenario. [NOTE: the bundled configuration uses 3 replicates per
scenario × severity cell. At this replicate count the per-cell
coverage estimates are noisy (baseline coverage rounds to 1.00
because all 3 CIs happened to cover); the qualitative pattern of
degradation across severity levels is reproducible but the
absolute coverage numbers should be regenerated at 200+ replicates
before they are used for inference. A `n_reps = 200` configuration
is provided in `inst/extdata/rescueco_dq_full.yml`.] The four
...
`survtmle`, `lmtp` [@lmtppackage2021], or the proprietary
`causalRisk` package as alternative implementations.
...
2. **MAR / MNAR missingness:** a native inverse-probability-of-
...
running. The proprietary `causalRisk` package
[@brookhart2024causalrisk] is an alternative implementation of
related cumulative-risk estimators and is referenced as such; the
two packages do not share code. The package is available at
...
published frameworks cleanTMLE operationalises in software, and
the authors of the proprietary `causalRisk` package (M. Alan
Brookhart, Alexander Breskin, and Bassim Eledath) for surfacing
the cumulative-risk-estimand framing that motivated several of
our diagnostic helpers. Any errors are ours.
