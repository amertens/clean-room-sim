...
Real-world evidence studies often require many analytic decisions
before an effect estimate is produced: how to define the study
population, how to handle limited overlap, which estimator to use,
and how to assess whether the data are reliable enough for the
question. In practice, these decisions are commonly documented in
separate places. The causal question and target-trial design may
be described in a protocol, data fitness may be assessed in a
checklist, and estimator choice may be justified in a statistical
analysis plan. This separation makes it difficult for reviewers to
see how design assumptions, data limitations, and estimation
...
observed treatment-outcome association. The package extends this
loop with a plasmode data-quality stress test: it generates
synthetic outcomes from the empirical covariate and treatment
structure and evaluates each candidate under prespecified threats,
including covariate missingness, exposure misclassification,
outcome misclassification, and unmeasured confounding.
...
correctly returned STOP, illustrating that the workflow flags
prespecified unmeasured-confounding scenarios under which the
planned analysis would no longer meet the locked
operating-characteristic thresholds. The data-quality stress test further showed that
outcome misclassification and unmeasured confounding produced the
largest degradation in estimator performance.
...
review, creating a quantitative bridge between the qualitative
data-fitness argument and the estimator-selection argument.

## What the package adds, and what it does not

cleanTMLE is an R [@rcoreteam2024] package that implements the
estimator-selection and stress-testing steps of an outcome-blind
staged workflow within the causal roadmap [@dang2023roadmap;
@petersen2014roadmap; @gruber2022sap]. Targeted maximum likelihood
estimation (TMLE) [@vanderLaan2006tmle; @vdL2011tlbook;
@schuler2017tmle; @zheng2011crossvalidated; @chernozhukov2018dml]
is the natural setting because every analytic decision (the
SuperLearner library [@vdL2007superlearner; @phillips2023sl], the
truncation rule, the prespecified set of candidate estimators) can
be listed in advance, and the algorithm remains data-adaptive while
still admitting valid Wald-type confidence intervals. TMLE is
particularly compatible with staged outcome-blind workflows because
the analyst can prespecify not only the causal estimand, but also
the nuisance-learning library, truncation rules, targeting
procedure, and candidate estimator grid. The resulting estimator
remains data-adaptive while preserving influence-function-based
inference under standard regularity conditions. This modular
structure makes it possible to compare prespecified candidates
under synthetic outcomes without repeatedly revising the analytic
plan after inspecting observed treatment-outcome associations. The package
adds an analysis-lock object, a serialisable audit trail and
decision log [@muntner2024staging], a pre-outcome decision rule
that classifies the analysis as GO (proceed to the locked primary
analysis), FLAG (proceed only with documented review or
qualification), or STOP (do not conduct or report the primary
comparative analysis under the current estimand/design/specification),
an outcome-blind plasmode candidate selector, and the DQ stress test
described above.

cleanTMLE implements the software layer of an outcome-blind staged
workflow. It records the analysis lock, candidate estimator set,
plasmode simulation settings, DQ stress scenarios, checkpoint
decisions, and final authorisation state. cleanTMLE authorisation
checks restrict the package's exported primary-analysis functions
until the required checkpoints have been recorded. These checks
do not control raw-data access outside the package and therefore
complement, but do not replace, institutional clean-room
safeguards such as role separation, credentialed folders, and
review-team checkpoints. The package does not implement personnel
separation, source-data access controls, protocol adjudication,
phenotype validation, or regulatory review.

The contribution is operational rather than methodological
[@muntner2024staging]. The analysis lock, audit trail, and
decision log turn paper-only prespecification checklists into a
structured record of analytic decisions that an external reviewer
can reconstruct. We use *outcome-blind staged workflow* throughout
to describe this arrangement. The "clean room" label is sometimes
used as a governance analogy, by analogy with a manufacturing
clean room that blocks access to a sterile area until protocols
have been followed, but the substantive content is the staged
restriction of outcome access. The software arrangement
complements, and does not replace, personnel-based access
controls, protocol review, role separation, source-data audit, and
independent checkpoint review. @sec-roadmap situates
the package inside the causal roadmap and @tbl-reg-align maps each component to the corresponding FDA,
EMA, and ICH expectation.
...
risk-difference** estimand, and outcome missingness handled through
**complete-case** and **IPCW** sensitivity paths under prespecified
missingness assumptions (a complete-case path that is unbiased under
MCAR, and an IPCW path that is unbiased under conditional
independent censoring given measured covariates and treatment). Within that scope, the package exposes the full
estimator spectrum: IPTW with stabilised weights
(`estimate_ipwrisk()`), parametric g-computation
(`estimate_gcomprisk()`), augmented IPW (`estimate_aipwrisk()`),
and TMLE (`run_clean_tmle()`, plus the modular `fit_tmle_*()`
primitives and the IPCW variant `run_ipcw_tmle()`). The
prespecified primary estimator is TMLE; the other three are
reported as concordance checks. Hazard ratios, time-varying treatments, competing risks,
...
should compose cleanTMLE's prespecification and audit functions
with `survtmle` [@survtmlepackage2018] or `lmtp`
[@lmtppackage2021] until those features are added.
:::

## What the paper contributes

The principal contribution is to extend the outcome-blind plasmode
loop from candidate-estimator selection [@dang2023roadmap;
@gruber2022sap; @schreck2024plasmode] to estimator robustness
*screening* under prespecified threat ranges. Each candidate
produces a degradation gradient on the same bias, RMSE, and
coverage scale as the baseline plasmode, so the prespecified
decision rule can be applied uniformly across baseline and
stressed scenarios. We position this screen as a quantitative
supplement to fit-for-purpose review (@sec-bridge) rather
than as a substitute for variable-validation studies, source-data
assessment, or post-outcome quantitative bias analysis. The R
implementation packages the supporting prespecification artefacts
(fingerprinted lock, audit log, decision log, pre-outcome decision
point) so that the screen and the locked primary analysis share a
single reproducible record. The next
section (@sec-roadmap) places these contributions inside the
...
@tbl-layers distinguishes the conceptual layers that
...
flowchart TD
    ...
    D --> E["5. Lock candidate estimators and decision thresholds<br/>cleanTMLE: create_analysis_lock, lock_primary_tmle_spec"]
    E --> F["Pre-outcome checks<br/>cleanTMLE: checkpoints, plasmode, DQ stress"]
    F --> G["Pre-outcome decision<br/>cleanTMLE: gate_check, authorize_outcome_analysis"]
    G -->|GO or FLAG| H["Unblind and run locked primary analysis<br/>cleanTMLE: four-step TMLE, IPCW-TMLE"]
...
```{mermaid}
%%| fig-cap: "Governance layers. The software-level arrangement is necessary but not sufficient for regulatory or confirmatory RWE."
flowchart LR
...
@tbl-reg-align maps each roadmap component to the
...
replace* column; that is the row that distinguishes
the software-level outcome-blind workflow from the broader
regulatory governance arrangement.
...
A complementary supplementary mapping (@tbl-event-diag)
lists the additional event-process and diagnostic structures the
package makes explicit. These rows are intentionally separated
from the main regulatory-alignment table to keep the latter
readable.
...
event_diag <- data.frame(
  ...
  `cleanTMLE function` = c(
    "clean_event_process_table",
    "clean_target_population",
    "clean_weight_diagnostics",
    "clean_check_event_processes",
    "clean_risk_report_table",
    "(documented preference; HR as secondary)"),
  Purpose = c(
...
knitr::kable(event_diag,
             caption = "Supplementary mapping. Event-process and diagnostic structures that the package makes explicit, paired with the helper that produces each one.")
```
...
prespecified and linked to sensitivity analyses; the package
provides `clean_event_process_table()` to record it as a
...
functions as a quantitative checkpoint. Rather than relying only
on qualitative discussion of data-fitness concerns, the workflow
evaluates whether candidate estimators continue to satisfy
prespecified operating-characteristic thresholds under threat
scenarios derived from the fit-for-purpose review.
...
until prespecified review steps have been completed. It should be
viewed as complementary to, not a replacement for, the personnel-
based governance arrangement. The audit and decision logs likewise
complement, rather than replace, the timestamped paper plan and
the data manager's role; reviewers see transparent documentation
of every analytic decision alongside the institutional
...
### Personnel-based and simulation-mediated protections are complementary

Personnel-based clean rooms protect against outcome-driven
protocol drift through role separation, staged access, and audit
of who can see what at each stage. Simulation-mediated blinding
supports prespecified estimator selection and stress testing using
synthetic outcomes generated from the empirical covariate and
treatment structure. The two arrangements target overlapping but
distinct residual risks: personnel-based workflows do not by
themselves quantify estimator fragility under data-quality
threats, while simulation-mediated workflows depend on the
fidelity of the synthetic-outcome mechanism. High-stakes studies
should consider using cleanTMLE inside a personnel-based
clean-room arrangement so that institutional outcome separation
and quantitative robustness screening apply together.
...
|---|---|---|
| Protocol and target-trial timing | `attach_estimand()` + target-trial timing table | No Y |
...
| GO / FLAG / STOP checkpoint dashboard | `gate_all()` composite + per-checkpoint records | No Y |
...
follows the guidance developed in the FIORD outcome-blind simulation
manuscript [@nance2026fiord], which formalises a five-step pre-outcome
candidate-evaluation procedure: (1) declare the candidate estimators,
...
- Propensity-score diagnostics [@austin2009ps] assess overlap,
  balance, and design feasibility, not estimator operating
  characteristics.
- Baseline plasmode simulation assesses candidate estimators under
  a *locked* synthetic outcome model using the empirical $(W, A)$
  structure.
- Negative-control analyses probe residual bias for selected NC
  outcomes through a different data-generating channel and do not
  characterise the behaviour of the primary estimator under DQ
  perturbations.
- DQ stress testing asks whether conclusions about estimator
  suitability *persist* under prespecified violations or
  degradations tied to fit-for-purpose concerns.
- A STOP under a stress scenario should be read as evidence that
  the study requires redesign, external validation, additional
  sensitivity analysis, or narrower causal claims before outcome
  access.

Four pre-outcome tools answer four different questions and are
complementary rather than substitutable. (i) Propensity-score
diagnostics assess empirical comparability, overlap, and weight
stability, indicating whether the design is feasible under the
observed covariate and treatment distributions; they do not
estimate the operating characteristics of a candidate estimator.
...
treatment-outcome association is inspected.
...
moves bias or coverage beyond the threshold. The candidate-selection
rule (`select_tmle_candidate(rule = "min_rmse")`) is currently a
single-step approximation of the **two-stage selector** formalised
in FIORD [@nance2026fiord]: in FIORD's procedure, candidates are
first screened to those that achieve nominal **oracle coverage**
(a CI built from the Monte Carlo empirical SE), and the point
estimator with the lowest variance among the survivors is locked
in; the variance method is then selected separately to achieve
nominal coverage on the locked point estimator. The current cleanTMLE
rule conflates these two stages, choosing on RMSE only; the
forthcoming `rule = "fiord_two_stage"` (planned for cleanTMLE 0.2)
implements the full two-stage procedure. The rule can additionally
be augmented to require a maximum allowed degradation in the DQ
stress test, which excludes fragile candidates before the outcome
is accessed.

The truncation level inside a TMLE candidate is more than a
variance-stabilising tuning knob. Trimming or truncating the
propensity score alters the effective target population by
restricting support to units with adequate overlap, and therefore
changes the estimand itself rather than only the variance of the
estimator [@lash2014qba]. When the
...
and coverage metrics as everything else rather than chosen
...
outcome-blind simulation literature [@nance2026fiord], makes the
admissibility argument concrete: every input to the candidate
...
*Cumulative-risk reporting.* `clean_risk_report_table()` produces a
compact table of cumulative risks at clinically meaningful follow-up
times, paired with risk-difference and risk-ratio summaries; the
`plot.cumrisk()` method renders the same content visually. The
package emphasises absolute risks at prespecified times rather than
ratio-only summaries.
...
effective sample size, weight percentiles, the maximum weight, and
...
*Event-process classification.* `clean_event_process_table()` and
`clean_check_event_processes()` distinguish event of interest,
competing event, censoring, treatment discontinuation or switching,
transfer-driven exclusions, and administrative end of follow-up,
and check component-vs-composite coherence so that follow-up
accounting is explicit rather than implicit. These helpers record
and check event-process structure; v0.1 does not yet estimate
...
censoring weights, and `clean_check_event_processes()` for
component-vs-composite coherence. Applied to a worked
...
cleanTMLE::clean_check_event_processes(ep, tolerance = 1e-3)
```
...
reflects this ordering: `clean_risk_report_table()` produces a
single structured table of cumulative risks, contrasts, and event
counts at the prespecified follow-up time, and hazard-ratio
helpers are provided only as secondary analyses with an explicit
hazard-scale estimand declaration.
...
# Add a G-value column (TODO A.22): the additive bias that would
# push the 95% CI across the null. Computed post-hoc from each
# row's mean estimate and mean SE.
for (sc in names(res$summaries)) {
  ...
  knitr::kable(
    ...
    caption = paste0(
      ...
      "95% CI across zero (Gruber 2023 RWE p.5); smaller values mean ",
...
The simulation illustrates three intended uses of the staged
workflow: confirming acceptable finite-sample behaviour, detecting
instability under marginal overlap, and stopping under a
prespecified data-generating process that encodes an identification
failure.

It is worth being explicit about what the workflow is and is not
showing. A STOP decision is **a failure of prespecified
operating-characteristic thresholds under the simulated DGP**, not
...
The pre-outcome decision returns STOP for Scenario C. Inspection of
the per-component thresholds shows that the trigger is the
unmeasured-confounding DQ row (estimated bias from the U-shift
scenario exceeds the prespecified maximum), rather than the
baseline plasmode coverage check: the baseline TMLE coverage in
Scenario C is at the lower edge of the prespecified range, and the
DQ supplement is what makes the failure unambiguous. This
illustrates the intended role of the DQ stress test as a
quantitative supplement to the baseline plasmode metrics.

A separate pattern concerns **variance-estimator selection**, the
second stage of FIORD's two-stage selector [@nance2026fiord], and is
worth surfacing because it routinely surprises applied users. The
mean-SE-to-empirical-SD ratio for IPTW is often above one in our
scenarios, because the default IF-based variance estimator treats
the propensity score as known and is therefore conservative when
the propensity score has actually been estimated
[@robins1992rotnitzky]. Doubly robust estimators (TMLE, AIPW,
CV-TMLE) do not share this conservativeness once the nuisance fits
are properly cross-fit; the SE/SD ratio for TMLE_CF stays near one
across our scenarios. The matching-with-TMLE composition shows a
similar pattern in Scenario B, where the SE-to-empirical-SD ratio
reaches roughly two and coverage exceeds nominal, because the
post-match TMLE step treats the matched cohort as if it were a
fresh independent sample, ignoring the residual variability
introduced by the matching draw.

The applied implication is consistent across the FIORD case study
[@nance2026fiord], the older Mertens–Zhang Danish-registry
investigation [@mertens2024danish], and our own results here: when
the prespecified primary is an IPW estimator, a matched-cohort
estimator, or any TMLE running on a non-i.i.d. sample, the
default IF-based variance is likely to be conservative and the
**nonparametric bootstrap variance** is the principled fix. The
bootstrap is therefore the recommended variance method for the
two-stage selector's second stage in cleanTMLE 0.2; the present
release supports it externally and surfaces the IF-based SE for
back-compatibility.
...
The same simulation data answer a different question if we read them as a workflow-vs-workflow contrast rather than an estimator-vs-estimator contrast. The decision-rule table below summarises what a traditional analyst would have reported under each scenario versus what the prespecified GO / FLAG / STOP rule returns. The estimates and CIs are unchanged; only the decision-rule overlay is new.
...
| **B. Marginal overlap** | Run prespecified TMLE; report. SE / SD ratio for matched-TMLE is 2.1 and for IPTW is 1.01 with conservative IF variance. Many reviewers will not catch this | **GO with FLAG** on IPTW and matched-TMLE SE calibration; the workflow flags both before unblinding and recommends bootstrap variance | The workflow surfaces the SE-conservativeness issue as part of the dossier; the traditional pipeline relies on the reviewer to catch it |
| **C. Unmeasured confounding** | Run prespecified TMLE; report a biased non-null estimate (mean bias 0.007, coverage 0.93). Without an external probe, this estimate would be published | **STOP** under the prespecified unmeasured-U DQ scenario; the locked analysis does not run as planned | The workflow blocks publication of the biased estimate; the traditional pipeline does not |
...
The workflow does *not* invent free bias reduction. In Scenario C the TMLE bias is the same under both pipelines (0.007); the workflow's contribution is the decision rule, not a different estimator.
...
the good-overlap and marginal-overlap scenarios. As implemented in
v0.1, this scenario applies MCAR missingness to a subset of the
locked covariates and reimputes with column medians, which is
approximately unbiased by construction under MCAR; the flat
gradient therefore confirms that median imputation does not
introduce additional bias under MCAR but is not a strong test of
the missingness handling. We discuss this limitation, and the
MAR / MNAR scenarios that would constitute a stronger test, in
@sec-limitations. Treatment misclassification attenuates the
...
for (sc in c("good_overlap", "marginal_overlap", "unmeasured_conf")) {
  pf <- sprintf("../results/plasmode_%s.csv", sc)
...
The case-study `glm_t01` and `glm_t05` candidates differ by < 0.001 on the simulation DGP because the propensity-score density never approaches 0 or 1, so both selectors typically agree. The contrast becomes informative when overlap is strained, when the candidate libraries differ in flexibility, or when severity ranges in the DQ stress test push some candidates off-coverage.
...
demonstration on a real registry structure, not as evidence that
Rescue.Co transport has a causal effect. The simulation in
@sec-simulation is small (5
...
(see @sec-casestudy-dq), so the Rescue.Co example is not
...
flowchart TD
    A["Registry observations<br/>(all 2018-2024 trauma transports)"]
...
| Stage 2c DQ stress test | `run_plasmode_dq_stress()` minimax rule | Each scenario within thresholds across severities | **FLAG** unless the $n_{\text{reps}} = 200$ DQ run is used (bundled $n_{\text{reps}} = 3$ is a workflow-demo only); GO for workflow execution | Selected candidate is stable across prespecified threats *as evaluated*; inferential interpretation requires the higher-replicate run | Software-enforced |
...
| 2026-01-12 11:02 | Stage 2c | DQ stress run at `n_sims = 3` for fast reproducibility | `n_sims` >= 200 for inferential interpretation | FLAG | Bundle low-replicate run as workflow demo; companion `n_sims = 200` file confirms minimax winner | Lead analyst | No | Use `n_sims = 200` for inference; cite `n_sims = 3` only as smoke test | No |
...
Both candidates pass the prespecified decision targets
...
envelope for the IPCW row. The Stage 2c DQ stress test sweeps the four threats
over four candidates; the minimax rule locks `glm_t01` as the
primary specification. [NOTE: the bundled
`config/clean_room_config.yml` sets `simulation.n_sims = 3`. The
bundled low-replicate run is retained only as a fast reproducibility
example; the case-study DQ results should not be interpreted
inferentially from `n_sims = 3`. The companion configuration
`inst/extdata/rescueco_dq_full.yml` runs the same sweep at
`n_sims = 200` and is provided to confirm the minimax winner.]
...
```{mermaid}
%%| fig-cap: "Negative-control attrition. Five negative-control endpoints were prespecified in the SAP. Three were dropped at the data-sanitisation step because they failed near-zero-variance (NZV) filtering on the locked cohort. The two surviving NCs (chronic_hypertension, household_urban) carried enough variation to fit a stable PS-adjusted model and both passed. The residual-confounding probe therefore rests on a narrower NC panel than originally specified; the audit log records the drops."
flowchart LR
...
(`run_plasmode_dq_stress()`) over the four threats and produces a
structural pass on each scenario. [NOTE: the bundled low-replicate
run uses 3 replicates per scenario × severity cell and is retained
only as a fast reproducibility example. The case-study DQ
results should not be interpreted inferentially from `n_sims = 3`.
A `n_reps = 200` configuration is provided in
`inst/extdata/rescueco_dq_full.yml` to confirm the minimax winner
under sufficient replicates.] The four
threats encoded by `run_plasmode_dq_stress()` (covariate MCAR,
exposure misclassification, outcome misclassification, latent $U$)
do not cover two design challenges that mattered in the rescueCo
...
a GO / FLAG / STOP classification of the analysis before
...
[@ich2024m14], as summarised in @tbl-reg-align. The
...
RMSE and also stays inside the prespecified decision thresholds across
...
cleanTMLE 0.1 implements a **single-step** candidate-selection rule
(`select_tmle_candidate(rule = "min_rmse")`, optionally augmented to
`"min_max_rmse"` over the DQ stress scenarios) that simultaneously
weighs point-estimator behaviour and variance-method behaviour
through a single RMSE / coverage / SE-window decision. The FIORD
outcome-blind simulation guidance [@nance2026fiord] argues that
these are distinct tasks and should be handled in two stages: first
screen point estimators by oracle coverage (a CI built from the
Monte Carlo empirical SE) and pick the lowest-variance survivor;
then select the variance method on the locked point estimator using
nominal CI coverage. **The current cleanTMLE rule is FIORD-inspired
but not FIORD-compliant.** The `rule = "fiord_two_stage"` selector
is planned for cleanTMLE 0.2; users who need full FIORD alignment
today should run the FIORD selector externally on the candidate
output of `run_plasmode_feasibility()`. This limitation affects the
main contribution and is not just a future-feature item.
...
missingness weighting, which weakens the assumption to stratified
MCAR; we plan to add a native cleanTMLE implementation in a future
release.
...
replicates. With the configuration of @sec-simulation (50
...
1. **FIORD two-stage candidate selector** [@nance2026fiord]:
   separate the point-estimator selection (oracle-coverage screen
   on the Monte Carlo empirical SE) from the variance-method
   selection, in place of the current single-step RMSE rule.
2. **Nonparametric bootstrap variance** as the default variance
   method for IPTW and matched-cohort estimators (the FIORD
   case-study winner; see also @mertens2024danish).
...
8. **MAR / MNAR missingness:** a native inverse-probability-of-
   missingness weighting path that does not require a structural
   missingness model.
...
decision rule can be applied uniformly to both. This is a
