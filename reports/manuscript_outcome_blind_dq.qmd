...
# Pre-outcome pass (Stages 1-3, outcome-blind). Builds the analysis lock
# (SHA-256 fingerprint of columns + data shape), runs cohort adequacy, PS
# overlap and balance, outcome-blind candidate selection and the DQ stress
# test, and the negative-control checks, and assembles the reviewer dossier.
...
pre <- run_clean_tmle_preoutcome(
  data = rescueco, Avar = "rescueco", Yvar = "gose_good",
  covariates = baseline_covariates,
  learner_lib = c("SL.glm", "SL.glmnet", "SL.gam"),
  tmle_candidates = cands, selection_rule = "min_max_rmse",
  dq_scenarios = default_dq_scenarios(),
  neg_control_outcomes = c("household_urban", "chronic_hypertension"),
  seed = 2026)
print(pre$dossier)
#> lock_hash 9f2c1a...; cohort 1,013 vs 680 per arm, 1,277 observed GOSE;
#> PS in [0.21, 0.81], max SMD 0.07; selected 'ensemble_t05' (min-max RMSE
#> across DQ threats); negative controls near null; decision: GO with FLAG

# Pre-outcome authorisation. The token binds the lock hash and an audit
# fingerprint; run_clean_tmle_primary() refuses to read the outcome unless it
# is handed a matching, authorising token.
auth <- authorize_outcome_analysis(pre$audit, allow_flag = TRUE)

# Stage 4: unblind and run the locked primary analysis under the token.
fit  <- run_clean_tmle_primary(pre, authorization = auth)
#> Risk difference 0.031 (95% CI -0.001, 0.062)
```

@tbl-functions summarises the functions the pre-outcome pass exercises,
what each one does, and the representative output it returns.
`run_clean_tmle_preoutcome()` runs the first block (Stages 1a-3) while the
outcome is masked; only `run_clean_tmle_primary()` (and the modular
IPCW and concordance siblings) reads the observed outcome, and it does so
only when handed an authorisation token that matches the lock hash and the
recorded audit. The unguarded `run_clean_tmle()` convenience wrapper, by
contrast, reads the outcome once its internal checks pass and is labelled
accordingly.
...
fns <- data.frame(
  ...
  `What it does` = c(
    ...
    "Run the locked four-step TMLE primary analysis on the pre-outcome pass; refuses to read the outcome without a token matching the lock and audit.",
...
and the analysis code is available in the companion repository. The
individual-level registry data are access-restricted (see the ethics and
data-availability statement below), so the case study is code-reproducible
rather than data-public. The DQ stress test in this run
is reported at 50 replicates (see @sec-casestudy-dq), an illustration
below the 200-replicate bar used for inferential interpretation. The example
...
| Stage 2c DQ stress test | `run_plasmode_dq_stress()` minimax rule | Each scenario within thresholds across severities | **GO** (reported on a $n_{\text{reps}} = 50$ illustrative run, below the 200 bar used for inference; the bundled $n_{\text{reps}} = 3$ configuration is a CI smoke test) | Selected candidate is stable across the prespecified threats at 50 replicates | Software-enforced |
...
| 2026-01-12 11:02 | Stage 2c | DQ stress reported at `n_sims = 50` (illustrative) | `n_sims` >= 200 for inferential interpretation | FLAG | Report the 50-replicate illustrative run; a 200-replicate confirmation is deferred as the case study is a workflow demonstration, not confirmatory; bundled `n_sims = 3` is a CI smoke test only | Lead analyst | Yes | Locked `ensemble_t05` as minimax winner (over `glm_t01`, which had the best baseline RMSE but the worst worst-case under the five-threat sweep) | No |
...
submission-grade analysis. The 3-replicate configuration bundled with the
staged pipeline is a continuous-integration smoke test and is not used
for inference.
...
produces a structural pass on each scenario at 50 replicates per threat-severity
cell (an illustration below the 200-replicate inferential bar; see
@sec-reproducibility for the entry point). The five
