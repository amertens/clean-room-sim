...
if (file.exists(boot_path)) {
  ...
  # Match_TMLE
  mt_A_ratio_if     <- .bv_def(boot_var, "Good",     "Match_TMLE", "se_sd_ratio_if")
  mt_A_cov_if       <- .bv_def(boot_var, "Good",     "Match_TMLE", "coverage_if")
  mt_A_cov_boot     <- .bv_def(boot_var, "Good",     "Match_TMLE", "coverage_boot")
  ...
  # IPTW
  iptw_A_ratio_if   <- .bv_def(boot_var, "Good",     "IPTW", "se_sd_ratio_if")
  iptw_A_cov_if     <- .bv_def(boot_var, "Good",     "IPTW", "coverage_if")
...
under marginal overlap. A supplementary variance comparison found that the stabilised IPTW
influence-function standard error is moderately conservative under
marginal overlap (mean SE/SD ratio ≈ `r round(iptw_B_ratio_if, 1)`,
95% CI coverage ≈ `r round(iptw_B_cov_if, 2)`), as expected when
the propensity score is treated as known, while the matched-cohort
TMLE IF-SE is well-calibrated under marginal overlap but slightly
anti-conservative under good overlap (coverage `r round(mt_A_cov_if, 2)`,
corrected to `r round(mt_A_cov_boot, 2)` by the nonparametric
bootstrap). This pattern motivates `select_variance_method()`: oracle
coverage on synthetic data correctly identifies the estimator-scenario
pairs where bootstrap provides a practical gain. When
...
for IPTW and Match_TMLE in Scenarios A and B. For each replicate, both
the IF-based and nonparametric bootstrap (B = 200 draws, GLM PS) SE are
computed; the table reports mean SE/SD ratios and 95% CI coverage under
each method.

In Scenario A (good overlap), both estimators are close to calibrated.
IPTW achieves se_sd_ratio_if ≈ `r round(iptw_A_ratio_if, 2)`, coverage_if =
`r round(iptw_A_cov_if, 2)`. Match_TMLE is slightly anti-conservative:
se_sd_ratio_if ≈ `r round(mt_A_ratio_if, 2)`, coverage_if =
`r round(mt_A_cov_if, 2)`. The bootstrap corrects this for Match_TMLE
(coverage_boot = `r round(mt_A_cov_boot, 2)`), illustrating a setting
where oracle coverage selection would favour bootstrap over IF.

In Scenario B (marginal overlap), the pattern reverses. IPTW shows the
theoretically expected conservatism: the IF-based SE treats the
propensity score as known rather than estimated [@robins1992rotnitzky],
and the wider PS range amplifies this effect (se_sd_ratio_if =
`r round(iptw_B_ratio_if, 2)`, coverage_if = `r round(iptw_B_cov_if, 2)`).
Match_TMLE, by contrast, is well-calibrated (se_sd_ratio_if =
`r round(mt_B_ratio_if, 2)`, coverage_if = `r round(mt_B_cov_if, 2)`).
A likely explanation: the nearest-neighbour caliper excludes
extreme-propensity-score units in Scenario B, restricting the
matched cohort to a sub-population with better effective overlap where
the i.i.d. assumption underlying the IF-SE is approximately satisfied.
For both estimators in Scenario B, the bootstrap tracks the IF closely
and provides no material improvement.

```{r boot-variance-table, eval = has_boot, results = "asis"}
#| label: tbl-boot-variance
#| tbl-cap: >
#|   IF-based versus bootstrap variance for IPTW and Match_TMLE,
#|   100 MC replicates per scenario, B = 200 bootstrap draws.
#|   se_sd_ratio is the mean SE divided by the empirical SD of
#|   the point estimate; values above 1 indicate conservative
#|   (overcovering) inference.
...
For Match_TMLE in Scenario A (good overlap), bootstrap corrects a modest
anti-conservatism in the IF-SE and would be preferred by oracle coverage
selection. For Match_TMLE in Scenario B (marginal overlap) and for IPTW
in both scenarios, the bootstrap provides no material improvement.
This result illustrates what `select_variance_method()` is designed to
do: rather than applying a universal rule (always bootstrap for matched
estimators; always use IF for IPTW), it uses oracle coverage on synthetic
data to identify the specific estimator-variance-method combinations that
achieve nominal coverage for the study's DGP and sample size. The
`bootstrap_rd_variance()` function provides the bootstrap SE when it is
selected.
