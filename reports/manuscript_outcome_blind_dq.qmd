...
if (file.exists(boot_path)) {
  ...
  # Fallback placeholders (replaced when CSV is present)
  mt_C_ratio_if  <- 0.87; mt_C_cov_if  <- 0.84; mt_C_cov_boot <- 0.95
  iptw_C_ratio_if <- 0.97; iptw_C_cov_if <- 0.94
  mt_A_ratio_if  <- 0.93; mt_A_cov_if  <- 0.89; mt_A_cov_boot <- 0.95
  iptw_A_ratio_if <- 0.97; iptw_A_cov_if <- 0.94
  mt_B_ratio_if  <- 1.04; mt_B_cov_if  <- 0.97
  mt_B_ratio_boot <- 1.05; mt_B_cov_boot <- 0.99
  iptw_B_ratio_if <- 1.23; iptw_B_cov_if <- 0.98
}
```
...
under marginal overlap. A supplementary variance comparison found that the matched-cohort
TMLE paired-difference SE is anti-conservative under good overlap
(95% CI coverage `r round(mt_A_cov_if, 2)`, roughly one failure per
`r round(1 / (1 - mt_A_cov_if))` intervals), because it ignores
matching-draw variance -- the variability from which controls are
selected when data are re-sampled (Abadie and Imbens 2008, 2016). A
full-pipeline nonparametric bootstrap that re-runs matching on each
resample corrects this to `r round(mt_A_cov_boot, 2)`. The stabilised
IPTW IF-SE is moderately conservative under marginal overlap (SE/SD
ratio ≈ `r round(iptw_B_ratio_if, 2)`) because it treats the
propensity score as known; correcting this requires an M-estimator,
not resampling. `select_variance_method()` uses oracle coverage on
plasmode-generated synthetic data to identify, for each
estimator-scenario combination, which variance method achieves nominal
coverage. When
...
TARGET [@cashin2024target] and START-RWE [@wang2021startrwe].
...
arrangement complements, but does not replace, the personnel-based
...
IPCW sensitivity paths under prespecified missingness assumptions (the
complete-case path is unbiased under MCAR, and the IPCW path is unbiased
...
time-varying treatments, competing risks, multinomial exposures, and
MAR/MNAR missingness models are out of scope for the v0.1 release;
...
review (@sec-bridge) rather than a substitute for variable-validation
...
and the locked primary analysis share a single reproducible record. The
next section (@sec-roadmap) places these contributions inside the causal
...
# Stage 1a: lock the analytic specification and attach the estimand.
...
nc <- run_residual_confounding_stage(lock,
        negative_controls = c("household_urban", "fuel_wood"))
...
#| label: tbl-functions
fns <- data.frame(
  Stage = c("1a", "1a", "1b", "2a", "2a", "2b", "2c", "2c", "3",
  ...
  `What it does` = c(
    ...
    "Record the five estimand attributes on the lock (kept in the audit trail, not the fingerprint).",
  ...
  `Representative output` = c(
    ...
    "lock with estimand fields populated",
...
conventionally summarised with standardised mean differences, but
...
`select_variance_method()`, with `bootstrap_rd_variance()` supplying the
native nonparametric bootstrap that is the principled choice for
stabilised IPTW and matched-cohort estimators. The rule can additionally
be augmented to require a
...
missingness fractions (0.05, 0.10, 0.20); two paired
treatment-ascertainment (sens, spec) cells (0.95, 0.99) and
(0.90, 0.95); the same two paired cells for outcome ascertainment;
and two paired unmeasured-confounder strengths
($(\text{OR}_A, \text{OR}_Y) \in \{(1.5, 1.5),\, (2.0, 2.0)\}$
with $U$ prevalence fixed at 0.20). Across the four threats this
yields nine severity cells per Monte Carlo scenario. The PS candidate
grid comprises two GLM-based specifications varying the truncation
threshold ($t \in \{0.01, 0.05\}$); regularised-learner PS
candidates are excluded from the stress-test grid because
SL.glmnet produces runaway computation on the near-positivity-
violation designs generated for Scenario B, and GLM and regularised
PS produce near-identical plasmode metrics on this DGP (see
@sec-limitations).
...
worth surfacing because it interacts with the DGP in unexpected ways.
@tbl-boot-variance reports a targeted 100-replicate variance comparison
for IPTW and Match_TMLE across three overlap scenarios: C (very good
overlap, minimal PS spread), A (good overlap), and B (marginal overlap).
For each replicate, both the IF-based and nonparametric bootstrap (B = 500
draws, GLM PS) SE are computed; the table reports mean SE/SD ratios and 95%
CI coverage under each method.

Scenario C (very good overlap) shows the strongest anti-conservatism for
Match_TMLE: se_sd_ratio_if ≈ `r round(mt_C_ratio_if, 2)`, coverage_if =
`r round(mt_C_cov_if, 2)`, while IPTW remains near-calibrated
(coverage_if = `r round(iptw_C_cov_if, 2)`). When PS spread is minimal,
almost every treated unit finds a close control match and the matching-draw
variance -- the variability from which controls are selected when the data are
re-sampled -- is largest relative to the outcome variance. The full-pipeline
bootstrap corrects Match_TMLE coverage to `r round(mt_C_cov_boot, 2)`.

In Scenario A (good overlap), the same pattern is present but less severe.
IPTW achieves se_sd_ratio_if ≈ `r round(iptw_A_ratio_if, 2)`, coverage_if =
`r round(iptw_A_cov_if, 2)`. Match_TMLE is anti-conservative: the IF-based
SE undercovers at `r round(mt_A_cov_if, 2)` -- meaning the 95% interval
fails in roughly one replicate in `r round(1 / (1 - mt_A_cov_if))` -- a
practically important gap in a discipline that requires transparent
uncertainty quantification. This anti-conservatism is theoretically expected:
the paired-difference SE conditions on the fixed matched dataset and ignores
matching-draw variance [@abadie2008bootstrap; @abadie2016matching]. The
nonparametric bootstrap corrects coverage to `r round(mt_A_cov_boot, 2)`.
This is the setting where `select_variance_method()` oracle coverage selection
would favour bootstrap over IF.

In Scenario B (marginal overlap), the IF-IPTW SE is theoretically conservative:
the IF-based variance treats the propensity score as known rather than
...
`r round(iptw_B_cov_if, 2)`). Notably, the bootstrap does not correct this
conservatism for IPTW: both bootstrap and IF coverage remain near
`r round(iptw_B_cov_if, 2)`. The reason is structural. The PS-estimation
...
Match_TMLE, by contrast, is well-calibrated (se_sd_ratio_if =
`r round(mt_B_ratio_if, 2)`, coverage_if = `r round(mt_B_cov_if, 2)`).
A likely explanation: the nearest-neighbour caliper self-selects a
sub-population with better effective overlap, where the i.i.d. assumption
underlying the IF-SE is approximately satisfied. For Match_TMLE in Scenario B
and for IPTW in both scenarios, the bootstrap provides no material improvement.

```{r boot-variance-table, eval = has_boot, results = "asis"}
#| label: tbl-boot-variance
#| tbl-cap: >
#|   IF-based versus bootstrap variance for IPTW and Match_TMLE across
#|   three overlap scenarios (C: very good, A: good, B: marginal),
#|   100 MC replicates per scenario, B = 500 bootstrap draws.
#|   se_sd_ratio is the mean SE divided by the empirical SD of
#|   the point estimate; values above 1 indicate conservative
#|   (overcovering) inference, values below 1 indicate anti-conservative
#|   (undercovering) inference.
disp <- boot_var[, c("scenario", "method", "emp_sd",
                     "se_sd_ratio_if", "coverage_if",
                     "se_sd_ratio_boot", "coverage_boot")]
names(disp) <- c("Scenario", "Method", "Emp. SD",
                 "SE/SD (IF)", "Coverage (IF)",
                 "SE/SD (boot)", "Coverage (boot)")
knitr::kable(disp, digits = c(NA, NA, 4, 3, 3, 3, 3))
```

The key practical implication is estimator- and scenario-specific, and
the theoretical basis for the bootstrap distinction is established in the
matching literature [@abadie2008bootstrap; @abadie2016matching]. For
matched estimators, the bootstrap is not merely one option among several: it
is the theoretically principled variance method because it re-runs the full
matching pipeline on each resample and therefore captures matching-draw
variance that the conditional IF-SE ignores. `bootstrap_rd_variance()` with
`estimator = "match_tmle"` implements this directly. The oracle coverage
evidence in Scenario A -- IF-coverage 0.89, bootstrap-coverage 0.95 --
validates this theoretical recommendation empirically. In Scenario B, where
the caliper restricts the matched cohort to a well-overlapped sub-population,
the matching-draw variance is less severe and the IF-SE happens to be
approximately calibrated; oracle selection would correctly identify this as a
setting where the two methods are equivalent. For IPTW, neither variance
method removes the conservatism from treating the PS as known; analysts who
require exact IPTW variance calibration should consider an M-estimator that
jointly models PS estimation and outcome weighting. This is what
`select_variance_method()` is designed to do: rather than imposing a
universal rule, it uses oracle coverage on plasmode-generated synthetic data
to identify, for the study's specific DGP and sample size, which variance
method achieves nominal coverage.
...
| **B. Marginal overlap** | Run prespecified TMLE; report. IPTW IF-SE is conservative (SE/SD ≈ 1.23) because the PS is treated as known; matched-TMLE IF-SE is approximately calibrated (SE/SD ≈ 1.04) in this scenario. Many reviewers will not catch the IPTW over-coverage | **GO with FLAG** on IPTW variance calibration; the workflow surfaces this before unblinding and notes that a matched-TMLE bootstrap is theoretically principled regardless of scenario | The workflow surfaces the variance calibration issue as part of the dossier; the traditional pipeline relies on the reviewer to catch it |
...
With the two-candidate GLM-only grid used in @sec-simulation, the
`glm_t01` and `glm_t05` candidates differ by less than 0.001 in
RMSE across scenarios, so both selectors land on the same candidate
in every case. The FIORD coverage screen is still informative as a
diagnostic: under marginal overlap (Scenario B) neither candidate
achieves oracle coverage within the 0.02 tolerance of nominal,
which the table flags as "no candidate within tol". This reflects
an inherent challenge of the near-positivity DGP rather than a
failure of the GLM library. The selector contrast becomes
substantively informative when the grid mixes learner libraries of
different flexibility, when the outcome surface is nonlinear, or
when severity ranges in the DQ stress test push some candidates
off-coverage.
...
prespecified GO/FLAG/STOP verdict in three scenarios, but it does not
...
cat(sprintf(paste0("Against the oracle, the screen with a flexible ",
  "(SuperLearner) synthetic-nuisance model attains sensitivity %.2f and ",
  "specificity %.2f; with a GLM synthetic-nuisance model these are %.2f and ",
  "%.2f. The gap between the two quantifies the cost of misspecifying the ",
  "synthetic-nuisance model, the central plasmode pitfall [@schreck2024plasmode].\n"),
  s[1], s[2], g[1], g[2]))
```

Two patterns make the screen honest rather than magical. First, the screen
...
recommend a flexible synthetic-nuisance model. The run reported here uses
@tbl-gate-oc's replicate count with the Monte Carlo standard error of each
bias estimate; the script's `reps` is set higher for a final run.
...
was executed at low replication (see @sec-casestudy-dq), so the example
illustrates a real-registry execution of the workflow rather than
serving as primary evidence of DQ-stress validity.
...
baselines <- data.frame(
  Capability = c(
    ...
    "Prespecification lock + audit trail + GO/FLAG/STOP gate",
...
is the principled choice when the influence-function variance is
conservative or invalid, namely stabilised IPTW (whose IF variance treats
the propensity score as known; @robins1992rotnitzky) and any
estimator run on a non-i.i.d. matched cohort. The remaining limitation is
practical rather than conceptual: the variance-method stage is exposed as
a separate call rather than folded into a single `run_*` wrapper, and the
bootstrap multiplies computation, so analysts reserve it for the
estimators where the IF variance is known to misbehave.
...
simulation in @sec-simulation therefore uses a two-candidate GLM PS
grid ($t \in \{0.01, 0.05\}$) for the DQ stress test; SL.glmnet is
...
replicates. With the configuration of @sec-simulation (30
replicates per cell, nine degraded severity cells plus one
undegraded baseline, and two PS candidates) the test fits 600
TMLE models per Monte Carlo scenario. For the three simulation
scenarios combined, this totals 1,800 models, which completes
in roughly three hours on a modern laptop. Larger candidate grids
or higher replicate counts scale linearly; parallelising over
severity cells is the most direct route for interactive use.
...
separate, and serves as a reference for which decisions are addressed at
...
@tbl-event-diag lists the additional event-process and diagnostic
structures the package makes explicit, paired with the helper that
produces each one.
