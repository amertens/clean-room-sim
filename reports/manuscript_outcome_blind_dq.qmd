...
simulated data to identify, for each estimator-scenario combination,
...
simulated outcomes will generalise to the realised outcome
process; the conclusions are only as credible as the
simulated-outcome model and the externally justified severity
...
missingness indicators, and simulated outcomes generated for
...
A standard staged analysis often separates three activities:
...
performance under one locked simulated outcome mechanism. cleanTMLE
...
compare prespecified candidates under simulated outcomes without
...
primary-analysis functions until every checkpoint had been recorded.
The current version deletes that layer. The lock's data frame never
holds the primary outcome, which lives in a separate store that only
the estimation stage joins back, and the one enforcement option that
remains, `enforce = TRUE`, makes estimation refuse to run until a named
approver restores the store, an approval the design log records
(@sec-framework). This software arrangement sits inside, rather
...
documents version 0.3.0 of the package, whose workflow surface is
sixteen verbs that run in stage order against one lock object:
`create_analysis_lock()`, `declare_negative_controls()`, and
`declare_estimand_ladder()` seal the plan; `fit_ps()`,
`assess_support()`, `estimand_feasibility()`, and `simulate_support()`
answer the estimability question; `define_candidates()`,
`stress_test()`, and `select_candidate()` choose the estimator under
declared threats; `negative_control_ladder()` and
`check_process_indicators()` look for residual confounding;
`design_report()` and `export_design_log()` produce what the review
team reads; and `estimate_effect()` and `run_estimand_ladder()`
estimate. Earlier versions accreted governance helpers faster than
they earned their place, and the 0.2.0 consolidation and the 0.3.0
revision removed them. The case-study estimates in @sec-casestudy were
...
calibration degrades when the simulated-outcome model is misspecified.
...
the same decision scale, and the lock, design-log, and pre-outcome-decision
record.
...
prespecification (the SHA-256-fingerprinted analysis lock, the design
log, and the pre-outcome decision) so that the screen and the locked
primary analysis share a single reproducible record.
...
lock, and recording every pre-outcome diagnostic in an exportable design
log, gives a reviewer a record that can be reconstructed without
...
no significant effect and publish a null that masks a real one. Under
this prespecified unmeasured-confounding scenario the realised
measured-covariate analysis is materially biased and under-covering, so
both a realised-bias oracle and the outcome-blind screen flag it
(detection and fragility agree), and a conventional pipeline would
publish a misleading null. The fits are identical under both pipelines;
the difference is the prespecified decision applied before the outcome
is read.
...
cross-fitting, and the candidates can be ranked on simulated data before the
...
instead of developing it in the methods, because the software
...
assessment and estimation through the same lock. A lock created with
`enforce = TRUE` refuses estimation until `unmask_outcome()` restores
the outcome store with a named `approved_by`, and that approval is a
design-log entry; whatever the setting, the lock's data frame never
holds the primary outcome.

```{r functions-table}
#| label: tbl-functions
fns <- data.frame(
  Step = c("lock", "lock", "lock", "design", "design", "design", "design",
           "select", "select", "select", "design", "design", "report",
           "report", "estimate", "estimate"),
  `Function` = c(
    "create_analysis_lock()",
    "declare_negative_controls()",
    ...
    "run_estimand_ladder()"),
  `What it does` = c(
    "Seal the design data, variable roles, learner library, seed, and the declared decision rules (dq_thresholds, nc_criteria) behind a fingerprint; move the primary outcome into a separate store; optionally require a named approval before estimation (enforce = TRUE).",
    "Register the negative controls with their confounding domains and, if not declared at creation, the criteria they will be graded against, as a design-log entry.",
    "Pre-register the primary estimand, the ordered fallbacks, the switching trigger, and sensitivity floors, as a design-log entry; later, attach the selected candidate.",
    ...
    "Declare the candidate TMLE specifications, singly or as a factorial grid over libraries and truncation.",
    "One outcome-blind plasmode loop: the clean-data baseline when no threats are declared, the data-quality sweep otherwise; carries the locked verdict and tipping points when the lock declares thresholds.",
    "Lock one candidate by a declared rule (baseline RMSE, worst-case RMSE across threats, or the FIORD two-stage screen).",
    "Fit every registered negative control on every nested cohort, name what turns null after a restriction, and grade each rung against the locked criteria.",
    ...
    "Release the design log, tidy or in the decision-log columns of the staged-analysis literature.",
  ...
  `Representative output` = c(
    ...
    "two controls registered with their domains; criteria recorded",
    ...
    "candidate set over libraries and truncation",
    "per-candidate bias, RMSE, coverage; degradation gradient per threat; verdict and tipping points",
    "the locked candidate with the rule that chose it",
    ...
    "decision-log table: date, stage, issue, decision, rationale, decided by",
    "one labelled estimate with SE, CI, p, and flags",
    "one labelled row per feasible estimand; logged switch"),
  check.names = FALSE, stringsAsFactors = FALSE)
knitr::kable(fns,
  caption = "The cleanTMLE workflow, one row per verb: what each does and representative output from the Rescue.Co protocol contrast. Everything above estimate_effect() runs with no outcome access.")
```
...
Stage 1a, as the `estimand` argument of `create_analysis_lock()`, and
is preserved on the lock object that downstream stages consume. The
estimand fields are metadata outside the integrity fingerprint, which
digests the design data and the analytic specification; the exported
design log carries the ladder declaration and every later switch, so
the estimand the analysis ends up reporting can be traced from the
declaration to the estimate.
...
structured object that the design log can carry.
...
| 2b | Outcome-blind support map and plasmode candidate selection | No (simulated outcome) |
| **2c** | **Plasmode DQ stress test** | **No (simulated outcome)** |
...
2b resamples the real covariates, redraws treatment from the fitted
propensity, and generates simulated outcomes from a covariate-only
baseline surface (fitted on the real outcome given covariates in the
hybrid generator mode, or supplied from pilot data in the external-pilot
mode) with the treatment effect injected, which is why the same loop
hosts both the support map of @sec-support and the DQ stress test of
@sec-dq.
...
pre-outcome diagnostics, decisions, deviations, and the named
unmasking approval in the design log before the observed primary
...
random seed, together with any decision rules declared at creation
(the data-quality thresholds `dq_thresholds` and the negative-control
criteria `nc_criteria`). A SHA-256 fingerprint of those fields and of
the content of every design column is recorded so that the same lock
can be reconciled later by an external reviewer; the primary outcome
is excluded from the digest, so masking and unmasking leave the hash
unchanged while any edit to the design data changes it. The estimand
ladder is attached to the lock by `declare_estimand_ladder()` and is
captured by the design log, not by the fingerprint, so any later
change to the ladder appears as a separate, timestamped entry. The
lock splits its input at creation: the data frame the design verbs
read holds design data only, and the primary outcome lives in a
separate store that the estimation stage joins back after its guard.
`mask_outcome()` removes the store, so a masked lock is physically
outcome-free wherever it travels, and the package's test suite asserts
that the design verbs return identical output on a masked lock and
under a permuted outcome; for the plasmode stage that identity holds
in the external-pilot generator mode, since the hybrid mode fits its
covariate-only baseline surface on the real outcome (see the tradeoffs
below).
...
and `export_design_log(format = "muntner")` writes it in that
template's columns (date, stage, issue, decision, rationale, decided
by). Earlier versions of the package additionally withheld the
estimation functions until an authorisation token matching the lock
and an audit record had been minted. Version 0.3.0 deletes that layer.
...
determined analyst could always have bypassed it outside the package.
What remains is one institutional switch: a lock created with
`enforce = TRUE` refuses estimation until `unmask_outcome()` restores
the outcome store with a named `approved_by`, and the design log
records who approved and when.

In addition to a single outcome-masking step, @muntner2024staging and
@kent2026fiord recommend tiered, role-based access in which
analytic advisors see summary outputs by stage and checkpoint
reviewers see summary outputs only by checkpoint. The design report
and the exported design log support that arrangement at the workflow
...
outcome variable. Reviewers and regulators evaluate two documents
...
data-quality scenarios may be evaluated using simulated outcomes
...
that arrangement: the design log sits
alongside the timestamped paper plan and the data manager's role, so
reviewers see transparent documentation of every analytic decision on
top of the institutional documentation that already exists.
...
investigator must defend the assumption that the simulated-outcome
...
simulated outcomes, so the analytic plan need not be re-frozen
...
recording which mode was used. The simulated outcome is generated
...
no independent check that the simulated-outcome mechanism is
...
| Component | cleanTMLE output | Outcome access |
|---|---|---|
...
| Outcome-blind support map | `simulate_support()` | Simulated Y only |
| Plasmode candidate selection | `define_candidates()`, `stress_test()`, `select_candidate()` | Simulated Y only |
| DQ stress-test results | `stress_test()` | Simulated Y only |
| Negative controls across cohort restrictions | `negative_control_ladder()` | Negative-control Y only |
| Care-process collider check | `check_process_indicators()` | No Y |
| Design log export | `export_design_log()` | No Y |

The dossier is the document the reviewer reads. Comparative
...
instead of living in a diagnostic file nobody reopens. The default
...
from the fitted propensity, and generates simulated outcomes from a
...
### Generate-treatment versus sample-treatment

One design choice in this simulation needs stating, because the
...
fitted propensity, and simulated outcomes follow. The sample-treatment
...
locked simulated outcome model conditional on the empirical
...
guarantee that estimator rankings under simulated outcomes will
...
#| tbl-cap: "Mapping data-quality concerns to plasmode-parameter inputs of stress_test()."
...
rule (`select_candidate(rule = "min_rmse")`) chooses on RMSE in a
...
not* probe. The table is a guide to interpreting `stress_test()`
output; the five scenarios do not constitute a complete threat
taxonomy.
...
A consequence is that the `stress_test()` output is
...
[@fda2024ehrclaims; @fda2021registries; @ema2026rwd] and outside
...
The thresholds are declared on the lock as `dq_thresholds`: a
maximum absolute bias on the risk-difference scale, a minimum
coverage, and a maximum RMSE ratio relative to the candidate's own
clean-data baseline, with optional FLAG levels for the last two. The
package ships no default, because the values belong to the study:
analysts should set them from the estimand magnitude, the
prespecified Type I error budget, and the operating characteristics
required by the receiver of the evidence, before the analysis lock is
signed. The simulation of @sec-simulation declares a bias bound of
0.02 and the case study one of 0.01, both with a coverage floor of
0.90. The lock fingerprints the declared values, and the verdict and
tipping points that `stress_test()` reports are pure functions of
those values, the declared threat grid, and the computed metrics, so
there is no reading-time judgment for the realised effect to steer.
...
`select_candidate()` rule, which picks the RMSE-minimiser on
...
under which this rule changes the locked estimator. Third, because the
baseline plasmode and the stressed runs are one `stress_test()` object
that carries the locked thresholds and the declared grid, and the
design log records their declaration, a reviewer can see the
externally derived ranges that went in, the prespecified thresholds
that came out, and reproduce the decision.
...
threat, at a different stage, with a different output.

## Why "outcome-blind" matters here

The plasmode generator and DQ stress test use all the data except
the outcome: real baseline covariates $W$, real treatment
...
The screen is a heuristic, not a guarantee. Passing the DQ stress test
establishes only that, *under the analyst's simulated-outcome model and
over the
...
simulated one did. Three caveats follow directly. First, the screen can
...
conclusions are conditional on the simulated-outcome model; if that model
...
simulated-outcome model with a SuperLearner library rather than a single
...
# Implementation and `stress_test()` {#sec-implementation}
...
# Stage 1a: lock the analytic specification with its decision rules,
# register the negative controls, and declare the estimand ladder. The
# estimand here is the marginal risk difference for the primary event
# by 24 months. The primary outcome moves into a separate store at
# creation; enforce = TRUE makes estimation wait for a named approval.
lock <- create_analysis_lock(
  data        = study_data,
  treatment   = "treatment",
  outcome     = "event_24",
  ...
  dq_thresholds = list(max_abs_bias = 0.01, min_coverage = 0.90,
                       max_rmse_ratio = 1.5),
  nc_criteria = list(null_band = 0.02),
  enforce     = TRUE,
  seed        = 2026
)
lock <- declare_negative_controls(lock, variables = "nc_outcome",
...
# computation on the near-positivity-violation simulated designs that
...
plas <- stress_test(lock, candidates,
                    effect_sizes = c(0.03, 0.05), reps = 200)

# Stage 2c: the same loop under the prespecified threat grid. The
# verdict and tipping points read the thresholds declared at Stage 1a.
dq_results <- stress_test(lock, candidates,
  threats = list(
...
dq_results$verdict
dq_results$tipping
selected <- select_candidate(dq_results, rule = "min_max_rmse")
lock     <- declare_estimand_ladder(lock, primary = "ATE",
                                    fallbacks = c("trimmed_ATE", "ATT",
                                                  "ATO"),
                                    candidate = selected)

# Stage 3: negative controls across cohort restrictions, graded against
# the locked criteria, and the care-process collider check.
nc   <- negative_control_ladder(lock, restrictions = cohort_rungs)
proc <- check_process_indicators(lock, indicators = process_vars)

# Pre-outcome decision: the dossier the review team reads, and the
# design log released with it.
design_report(lock, sup, fea, simulation = sim, nc_ladder = nc,
              dq = dq_results)
export_design_log(lock, format = "muntner")

# Stage 4: estimation. The outcome store is restored by a named
# approver, which the design log records; the ladder then runs the
# declared primary when feasible plus every feasible fallback.
lock <- unmask_outcome(mask_outcome(lock), lock,
                       approved_by = "design review panel")
fit  <- run_estimand_ladder(lock, ps)
fit
...
Two pieces above are new relative to conventional targeted-learning
plumbing: the support and feasibility stage of @sec-support, and the
DQ stress test. `stress_test()` returns the same object class whether
or not threats are declared, and both feed the same decision, so
adding the data-quality sweep to the workflow does not require new
...
for each (scenario, severity) in scenario grid:
  for each replicate r:
    ...
    generate simulated outcome from the lock's covariate-only Q0 model
...
for reproducibility. First, each replicate runs under its own seed,
derived from the lock seed as `lock$seed + rep_i + sg_i * 10000L`
through `withr::with_seed()`, so that re-running the same lock with
the same scenario list produces bit-identical metrics, and a run
parallelised over replicates (`parallel = TRUE`, under the caller's
`future::plan()`) is identical to the sequential run.
...
and the `specify_models()` constructor allow
...
as code objects instead of prose. This lets the analysis lock
...
with risk-difference and risk-ratio summaries, and puts absolute
risks at prespecified times ahead of ratio-only summaries.

*Censoring and missingness weights as a first-class analytic object.*
`estimate_effect(missing = "ipcw")` carries
inverse-probability-of-censoring weights through the targeting step,
and `clean_weight_diagnostics()` exposes
...
follow-up accounting is explicit. It records and checks event-process
structure, and the cumulative-risk grammar's inverse-probability
estimator reports cause-specific cumulative incidence when a competing
event is identified; the staged support-and-ladder machinery does not
yet extend to those estimands (@sec-discussion).
...
checks through `clean_weight_diagnostics()` for treatment and
...
## Reporting cumulative risks before hazard ratios
...
reaches the boundary. This gives 19 severity cells per Monte Carlo
...
failure of prespecified operating-characteristic thresholds under the
simulated DGP; it is not detected real-data bias. The clarity of the
STOP decision in
...
bias bound than the `max_abs_bias = 0.01` used in the case study. The
wider band is chosen so that the illustrative
...
here corroborates the baseline plasmode metrics without rescuing them.
...
repairs this: bootstrap coverage rises to
...
The practical implication is estimator- and scenario-specific.
...
oracle coverage on plasmode-generated simulated data to identify, for the
...
because under them median imputation is biased as well as
...
`select_candidate()` ships three selection rules. The default `min_rmse`
...
prespecified threat can do; the third, `fiord_two_stage`, is the coverage
screen of @sec-limitations. The simulation grid in @sec-simulation does not
...
than claiming it is the common case. The full sweep runs through the
plasmode engine behind `stress_test()`, first without threats and then
with `near_positivity` declared alongside the others, and through the
selection rules of `select_candidate()`, on the same `run_simulation.R`
...
if (!file.exists(div_path) || !file.exists(div_fig) || !file.exists(div_tab)) {
  stop("Divergence-study files not found under sandbox/candidate_divergence/. ",
...
simulated replicates per cell and five independent batches, and reports Monte
...
margin within one or two Monte Carlo standard errors should be read as a
tie.
...
the worst-case rule selects it. The illustration is constructed, other grids
need not separate the rules, and the demonstration does not weaken the
prespecification logic of the workflow. The threat list and its severities are declared in the locked
...
degrades when the simulated-outcome model is misspecified. We answer this
...
what-if confounder of strength $s$ is injected into *both* the simulated
treatment and the simulated outcome, and the same TMLE is scored against
the simulated truth. The simulated nuisances are fitted two ways, a
...
confounder, and the screen can predict that bias only if its simulated
...
knitr::kable(
  ...
  caption = paste0("Calibration of the pre-outcome gate. Oracle bias is the ",
    ...
    "under a GLM and a flexible (SuperLearner) simulated-nuisance model. ",
...
cat(sprintf(paste0("Against the oracle, over all %d cells the screen with a ",
  "flexible (SuperLearner) simulated-nuisance model attains sensitivity %.2f ",
  "and specificity %.2f; with a GLM simulated-nuisance model these are %.2f ",
  ...
  "simulated-nuisance model, the central plasmode pitfall [@schreck2024plasmode].\n"),
...
simulated treatment and outcome surfaces are too simple to reproduce the
...
recommend a flexible simulated-nuisance model. Two limitations of this
...
illustration; they are not confirmatory evidence about the causal effect
of Rescue.Co transport. The simulation in @sec-simulation
...
data-availability statement below), so the case study can be reproduced
from its code but not from public data. The DQ stress test reported
...
result files are excluded from version control for this reason. The
...
@sec-support carries the analysis; (b) the
...
encoded by `stress_test()`; and (e) five prespecified
...
estimate and labelled as such instead of dropped silently
...
design report says so in words [@crump2009trimming].
...
contrast over the prespecified surface family (simulated confounding
...
the bounded estimands sit at 0.95), before any simulated confounding
...
simulated confounding strengthens the unmatched estimands degrade
...
reads as confounding-strength stress; a positivity problem would have
shown at the mildest surface, and the PASS verdict says there is none. These two patterns are
...
stressed outcome surface (simulated confounding strength 2), 200
...
produced by run_estimand_ladder() on the locked designs. C2 carries
no ATE row; the C1 ATE row is a logged reconciliation override and
carries its SEVERE verdict. Ladder switches and the override are in
the design log.")
} else {
...
The substantive pattern is coherent across the ladder. The
within-ambulance contrasts favour Rescue.Co on six-month mortality:
the C4 primary row is $-2.9$ percentage points (95% CI $-4.9$ to
$-0.9$), and on the primary cohort the effect concentrates among the
treated (ATT $-2.8$, CI $-4.8$ to $-0.9$; ATE $-1.7$, CI $-3.5$ to
$0.1$), with the overlap-weighted rows nearest the null. The
against-everyone contrasts C1 and C2 are null on every feasible
estimand. C3, any ambulance against non-ambulance arrival, shows
higher mortality (+1.8 points, CI 0.8 to 2.8), and the design stage
supplies its reading: both socioeconomic negative controls fail on
that contrast with no pre-registered restriction that repairs them,
and its care-process indicators are treatment-predicted, so the
design report carries C3 as residually confounded by indication
rather than as a harm signal. On time to arrival, the population
contrasts agree that ambulances, and Rescue.Co in particular, reach
patients three to four and a half hours sooner (for example C3
$-245$ minutes, CI $-320$ to $-169$); the transfers-included C4 gap
of $-100$ to $-205$ minutes shrinks to a null $-0.1$ minutes (CI
$-32$ to $31$) on the primary cohort, so the gap is carried by the
transfer and prior-care strata that the primary cohort excludes. No
row anywhere tripped the implausibility guard.

Two readings of this table concern the workflow itself. First, the severe-overlap contrasts do not
answer the protocol's original question. C2 carries no ATE row at
all; C1 carries exactly one, estimated under a logged override whose
recorded reason is reconciliation (the parallel unblinded pipeline
reports the C1 ATE as a SEVERE-flagged diagnostic, and the row lets
the two pipelines be compared), and it arrives wearing its SEVERE
label with a standard error several times larger than the bounded
rows beside it. The substantive C1 and C2 answers are the trimmed,
treated-population and overlap-weighted effects, each labelled with
the population it is about. What a conventional pipeline would have
reported for C1, an IPTW or TMLE ATE under weights reaching
`r round(.sup("C1", "max_iptw_weight"))` and no label, is exactly the kind of
...
files. The healthcare-worker strike period remains an unmodelled
threat that the prespecified threat families do not cover, and we
carry it as an unquantified limitation.
...
unblinded analysis surfaced this because the earlier version's audit
trail had preserved both records: the STOP and the estimate, with no
...
estimation. We report this because it is the working theory
...
runs the full DQ stress test (`stress_test()`) over the
...
`stress_test()` (covariate missingness under
...
(@sec-support-map), and generates simulated outcomes from a
covariate-only model $Q_0(W) = E[Y \mid W]$, so the simulated covariate
...
treatment-outcome law. The simulated outcome carries a fixed risk difference
...
diagnostic is necessary for fidelity without being sufficient, consistent
...
releases addressed: `stress_test()` failed outright on a
...
governance layer grew faster than its evidential value. The September
2026 revision (0.3.0) went further in the same direction: the workflow
surface is sixteen verbs, the checkpoint and authorisation-token layer
is deleted, the primary outcome lives in a store the lock's data frame
never holds, and the decision rules the verdicts read are declared and
fingerprinted at lock creation. We document both directions
because the working theory of the framework
is that the prespecification record should make problems visible,
including problems with the package itself, instead of absorbing them
into a hand-edited final table.

The full release notes are in `cleanTMLE/NEWS.md`, and the case
study's `results/multiarm/` directory contains the CSV files
...
used as one layer in a broader governed workflow. The lock and the design
log track the prespecification and traceability that FDA RWE program guidance,
...
cleanTMLE workflow records each declared NCO and its domain in the
lock and the design log, so the chosen domains are part of the record.
...
simulated outcome model and degradation mechanisms used to
...
simulated-outcome model used by the baseline plasmode. If that
...
both stages. Stage one is `select_candidate(rule = "fiord_two_stage")`,
...
compares variance methods by their oracle coverage on simulated data and
...
and the simulated effect is constant on the risk-difference scale
...
`stress_test()` takes a `q0_library` argument so the analyst can fit
Q0 with a SuperLearner library in place of the default logistic GLM;
this relaxes the bias of candidate selection toward linear-in-logit
learners when the underlying outcome surface is nonlinear. A
companion pilot simulation (`plasmode_selection_paper/`) provides
the empirical motivation: at n = 500, a logistic-GLM Q0 produces
...
The DQ stress test generates simulated outcomes under degraded
covariate, treatment, and confounder distributions, and then re-fits
every candidate PS model on each simulated dataset. The simulated
...
1. **Single-call FIORD alignment.** Both FIORD stages are now
   implemented (`select_candidate(rule = "fiord_two_stage")` for the
   ...
   wrapper and caching the bootstrap would make full FIORD alignment a
   one-call default in place of two explicit steps.
...
5. **DGP-spectrum argument** for the plasmode generator: structural-
   ...
   parametric-bootstrap modes, with simulated-data fidelity metrics
   (Hellinger / Wasserstein, or covariate SMDs between real and
   simulated samples) so the analyst can judge whether the DGP is
   faithful enough to base candidate selection on.
6. **Matched-cohort vs full-cohort plasmode diagnostic:** under a
   known simulated truth, compare the two estimators side by side
   to diagnose whether observed real-data gaps are sampling noise
   or positivity-driven.
7. **Richer simulated-outcome models:** effect modification, time-
...
9. **Survival and competing-risk estimands:** wrapping `survtmle`
   ...
   and design-log machinery so that hazard-ratio and cumulative-
...
11. **Calibration helpers for severity ranges:** small helpers that
    ...
    `threats` lists for `stress_test()`, with the citations attached
    to the design log.
...
analysis lock together with the exported design log. For
...
prespecification is filed in the same form that the design log will
later reference.
...
4. Run `stress_test()` on the surviving estimand, first with no
   threats and then with the SPIFD2-derived ranges declared as
   `threats`, and lock the candidate with `select_candidate()`.
...
The cleanTMLE package ships
`r length(list.files("../cleanTMLE/tests/testthat", pattern = "^test-.*[.]R$"))`
testthat test files covering the core workflow: analysis-lock creation and validation, the support assessment
...
push through the repository's continuous-integration configuration
...
In addition to the unit tests, we validate the package's targeted-learning estimator
...
| `sandbox/rescueco_fidelity/plasmode_fidelity.R` | `rescueCo/results/plasmode_fidelity.csv` | Case-study simulated-outcome fidelity subsection |
...
reg_align <- data.frame(
  ...
  `What cleanTMLE operationalises` = c(
    ...
    "define_candidates, select_candidate, save_lock",
    "stress_test, simulate_support",
    "design_report, export_design_log",
    "estimate_effect, run_estimand_ladder",
    "negative_control_ladder, check_process_indicators",
