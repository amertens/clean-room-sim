...
real covariate distribution and the real treatment. The observed
primary treatment-outcome association is not read; depending on
the configured mode, the plasmode generator may use marginal
outcome information (the covariate-only mean model $E[Y \mid W]$)
without using the joint $(A, Y) \mid W$ relationship, or it may
take $E[Y \mid W]$ from external pilot data
...
Doubly robust estimators (TMLE, AIPW,
...
function falls back to weighted complete-case).
