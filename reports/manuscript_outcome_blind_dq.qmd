...
TMLE with a flexible library. Provisional results at smoke scale,
to be confirmed at the full replicate count, show the crude
estimator biased by roughly $+0.13$ on a true risk difference near
$-0.08$, so it carries the wrong sign; the GLM-based TMLE removes
much of the confounding but retains a bias near $+0.04$ with
coverage about $0.67$; and the cross-fitted TMLE with a
generalised-additive-model-plus-interaction library is
approximately unbiased, with bias near $+0.01$ and coverage at the
nominal level. This is the one scenario in which the flexibility of
the nuisance learner, rather than the staging or the data-quality
screen, is decisive. It answers the question a methods reviewer
would press hardest, namely whether targeted learning with an
adaptive library earns its place over a parametric adjustment on
this class of problems; under a correctly specified linear model
(Scenarios A to C) it does not need to, which is why that contrast
requires a separate, deliberately nonlinear design.
...
IPTW, TMLE, cross-fitted TMLE, and Match_TMLE across three overlap regimes. Overlap regime C has minimal
...
the cross-fitted TMLE, and they are included here because the main
simulation showed them anti-conservative in that regime. Provisional
results at smoke scale, to be confirmed at the full replicate count, show
both with an influence-function SE/SD ratio near $0.85$ and coverage near
$0.87$ under marginal overlap, and the nonparametric bootstrap does
**not** repair them: bootstrap coverage stays near $0.87$ with an SE/SD
ratio near $0.88$. The reason is that part of the shortfall is finite-sample
bias toward larger effects near the positivity boundary, not variance
understatement, and a percentile bootstrap centred on the point estimate
cannot correct bias. The practical implication is that under marginal
overlap the remedy is not a different variance estimator but a positivity
diagnostic and, if warranted, heavier truncation or a restricted target
population; this is the same lever the candidate-divergence analysis
(@sec-divergence) selects on.
...
`run_plasmode_dq_stress()` over the prespecified threat sweep on a wider
...
envelope for the IPCW row. The Stage 2c DQ stress test sweeps the prespecified threats
...
and near-positivity) do not cover two design challenges that mattered
in the rescueCo
...
difference is known. Provisional results, to be confirmed on the final run,
show the cleanTMLE cross-fitted TMLE and `tmle::tmle()` agreeing to roughly
four decimal places (a difference near $0.0002$ in the risk difference, with
...
| `run_gate_operating_characteristics.R` | `results_new/gate_oc.rds` | @sec-gate-oc figures |
