...
This paper presents cleanTMLE, an R package that implements a staged,
outcome-blind targeted-learning workflow for binary point-treatment studies.
The package provides a fingerprinted analysis lock, an audit trail and decision
log, outcome-blind plasmode candidate selection, a plasmode data-quality stress
test with a first-class positivity-violation threat, and a staged pre-outcome
decision, with a reproducible script-to-output record for every reported result.

The workflow addresses a recurring problem in real-world evidence. The many
analytic decisions made before an effect estimate is produced, how to define the
study population, how to handle limited overlap, which estimator to use, and how
to assess whether the data are reliable enough for the question, are commonly
documented in separate places, which makes it difficult for reviewers to see how
design assumptions, data limitations, and estimation choices are connected.
cleanTMLE records these decisions as one machine-checkable object.
...
The boundary with the FIORD framework is as follows. FIORD
...
## Demonstrating that the minimax rule changes the decision {#sec-divergence}

The grid above does not separate the candidates, so it cannot show the
`min_max_rmse` rule selecting a different candidate from `min_rmse`. We
therefore report a constructed demonstration whose purpose is existence: it
...
The package documents its application programming interface in two vignettes,
`cleanTMLE-staged-analysis` and `cleanTMLE-functions`, and a generated function
reference. The staged-analysis vignette walks through the lock, the staged
checkpoints, outcome-blind candidate selection, the DQ stress test, the
pre-outcome decision, and the locked primary analysis as a single runnable
script. New users should begin there.
