...
# Numbers and figures come from the multi-arm result files; the effects
# slides fill in as the estimation chain writes its outputs.
library(ggplot2)
...
2. The design stage, in five verbs.
...
lock <- create_analysis_lock(data, "treatment", "outcome", covariates,
                             negative_controls = ncs, mask = TRUE)
lock <- declare_estimand_ladder(lock, primary = "ATE",
                                fallbacks = c("trimmed_ATE", "ATT", "ATO"),
                                trigger = "SEVERE")

ps  <- fit_ps(lock)                # SuperLearner, GLM, or external
sup <- assess_support(ps)          # graded overlap verdict
fea <- estimand_feasibility(ps)    # which estimands survive this fit
sim <- simulate_support(lock, ps)  # the outcome-blind support map

design_report(lock, sup, fea, simulation = sim)
fit <- run_estimand_ladder(lock, ps)   # outcomes join only here
```

Everything above the last line runs with no outcome access.
...
So the package simulates: covariates resampled, treatment redrawn from the fitted propensity, synthetic outcomes drawn from a prespecified family of surfaces. The truth is known on every replicate, separately per estimand. The map shows where each estimand holds a bias tolerance at near-nominal coverage.
...
Candidate TMLE specifications are compared on synthetic outcomes, then re-run under prespecified data-quality threats: covariate missingness (MCAR, MAR, MNAR), exposure and outcome misclassification, near-positivity, unmeasured confounding.
...
| Estimator selection | FDA non-interventional studies; ICH M14 | `tmle_candidate()`, `select_tmle_candidate()` |
| Pre-outcome DQ stress | Source reliability (FDA, EMA) | `run_plasmode_dq_stress()`, `simulate_support()` |
