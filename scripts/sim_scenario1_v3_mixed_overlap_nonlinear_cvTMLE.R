#!/usr/bin/env Rscript
# ===========================================================================
# Scenario 1 v3: Mixed Overlap, Nonlinear Outcome, CV-TMLE
# ===========================================================================
#
# PURPOSE:
#   Demonstrate "false stop" and "true stop" under a clean-room feasibility
#   gate. Mixed replicate-level overlap regimes (ok/bad) combined with two
#   modes of outcome dependence on the overlap-driving covariate (weak/strong
#   W4) produce a 2x2 grid:
#
#     +-------------------+-----------+-----------+
#     |                   | Weak W4   | Strong W4 |
#     +-------------------+-----------+-----------+
...
#     +-------------------+-----------+-----------+
#
#   The nonlinear outcome model ensures that misspecified GLM estimators
#   (crude, adjusted regression, TMLE-GLM) have nontrivial bias, while
#   TMLE-ML and cross-fitted TMLE with flexible learners can recover.
#
# CLEAN-ROOM STAGES:
...
# ESTIMATORS:
#   1. Crude RD
#   2. Adjusted regression (misspecified GLM g-computation, bootstrap CI)
#   3. IPTW RD (Hajek, bootstrap CI)
#   4. PS-matched regression (MatchIt NN, pairs bootstrap CI)
#   5. TMLE-GLM (misspecified, IC + multiplier bootstrap)
#   6. TMLE-ML (richer SL library, IC + multiplier bootstrap)
#   7. Cross-fitted TMLE-ML (V-fold, IC + multiplier bootstrap)
#
# USAGE:
#   Rscript scripts/sim_scenario1_v3_mixed_overlap_nonlinear_cvTMLE.R
#
# OUTPUT:
#   results/scenario1_v3/{weak,strong}/ — truth, per-rep, summary, figures
# ===========================================================================
...
source(file.path("R", "sim_helpers_v3.R"))
...
params_base <- list(
  ...
  s_bad = 2.0,          # treatment-strength multiplier for BAD overlap
  ...
  # h1(W1) = 0.6*W1 + 0.4*W1^2
  # h2(W2) = 0.7*W2
  # h3(W3) = 0.5*abs(W3)
  # h4(W4) = beta4_Q * W4
  beta0_Q   = -0.5,     # intercept (targets ~25-35% event rate)
  betaA_Q   = 0.40,     # treatment main effect (log-odds)
  tau_int1  = 0.25,     # A * I(W1 > 0)
  tau_int2  = 0.10,     # A * W2
  # beta4_Q and tau_int3 are mode-specific, set below
...
mode_params <- list(
  weak = list(
    beta4_Q  = 0.05,     # W4 barely affects Y
...
run_one_rep_v3 <- function(r, p, truth_rd) {
  rep_seed <- p$master_seed + r

  # Generate data (overlap regime drawn randomly)
  set.seed(rep_seed)
  sim <- sim_data_v3(n = p$n_main, seed = rep_seed, p = p)
  dat <- sim$data
  ...
  # ------------------------------------------------------------------
  ps  <- estimate_ps_v3(dat, p)
  diag <- overlap_diagnostics_v3(ps$g_raw, ps$g_bounded, A, p)
  ...
  # (2) Adjusted regression (misspecified, bootstrap CI)
  set.seed(rep_seed + 200000L)
  regadj <- est_reg_adj_v3(dat, B_boot = p$B_regboot)

  # (3) IPTW
  set.seed(rep_seed + 300000L)
  iptw <- est_iptw_v3(Y, A, ps$g_bounded, dat, B_boot = p$B_iptw, p = p)

  # (4) PS-matched regression
  set.seed(rep_seed + 400000L)
  psmatch <- est_psmatch_reg_v3(dat, ps$g_bounded, B_boot = p$B_matchboot, p = p)
  ...
  tmle_ml <- est_tmle_ml(Y, A, W, p)

  # (7) Cross-fitted TMLE-ML
  set.seed(rep_seed + 700000L)
  cvtmle <- est_cvtmle_ml(dat, p, V = p$V_cf)

  # ------------------------------------------------------------------
  # Multiplier bootstrap for each TMLE variant
  # ------------------------------------------------------------------
  ...
  if (tmle_ml$success && p$B_multboot > 0) {
    set.seed(rep_seed + 810000L)
    mb_ml <- multiplier_bootstrap(tmle_ml$estimate, tmle_ml$eif, p$B_multboot)
  }
  ...
  data.frame(
    ...
    # TMLE-GLM multiplier bootstrap
    tmle_glm_se_mb = mb_glm$se,
    tmle_glm_ci_mb_lo = mb_glm$ci_low, tmle_glm_ci_mb_hi = mb_glm$ci_high,

    # TMLE-ML (IC-based)
    tmle_ml_est = tmle_ml$estimate, tmle_ml_se_ic = tmle_ml$se,
    tmle_ml_ci_ic_lo = tmle_ml$ci_low, tmle_ml_ci_ic_hi = tmle_ml$ci_high,
    tmle_ml_success = tmle_ml$success,
    # TMLE-ML multiplier bootstrap
...
run_mode <- function(mode_name, params_base, mode_params) {
  ...
  p <- params_base
  p$beta4_Q  <- mode_params[[mode_name]]$beta4_Q
  p$tau_int3 <- mode_params[[mode_name]]$tau_int3
  ...
  truth <- compute_truth_v3(p)
  saveRDS(truth, file.path(mode_dir, "truth.rds"))
  message(sprintf("  RD_true = %.6f  (EY1=%.4f, EY0=%.4f)",
                  truth$RD_true, truth$EY1, truth$EY0))

  # --- Replicate loop ---
  message(sprintf("Running %d replicates (N=%d, p_bad=%.2f, beta4=%.2f, tau_int3=%.2f) ...",
                  p$reps, p$n_main, p$p_bad, p$beta4_Q, p$tau_int3))

  results_list <- vector("list", p$reps)
  for (r in seq_len(p$reps)) {
    results_list[[r]] <- run_one_rep_v3(r, p, truth$RD_true)
    ...
    if (r %% 50 == 0 || r == 1) {
      rr <- results_list[[r]]
      message(sprintf("  Rep %d/%d  regime=%s  flag=%s  TMLE-ML=%.4f  CV-TMLE=%.4f  crude=%.4f",
                      r, p$reps, rr$overlap_regime_true, rr$overlap_flag,
                      ifelse(is.na(rr$tmle_ml_est), NA, rr$tmle_ml_est),
                      ifelse(is.na(rr$cvtmle_est), NA, rr$cvtmle_est),
  ...
  build_metrics <- function(df, label) {
    rbind(
      compute_metrics_v3(df$crude_est, df$crude_ci_lo, df$crude_ci_hi,
                         df$crude_success, truth$RD_true, df$crude_se,
                         label, "Crude"),
      compute_metrics_v3(df$regadj_est, df$regadj_ci_lo, df$regadj_ci_hi,
                         df$regadj_success, truth$RD_true, df$regadj_se,
                         label, "Adj. Regression"),
      compute_metrics_v3(df$iptw_est, df$iptw_ci_lo, df$iptw_ci_hi,
                         df$iptw_success, truth$RD_true, df$iptw_se,
                         label, "IPTW"),
      compute_metrics_v3(df$psmatch_est, df$psmatch_ci_lo, df$psmatch_ci_hi,
                         df$psmatch_success, truth$RD_true, df$psmatch_se,
                         label, "PS-Matched"),
      compute_metrics_v3(df$tmle_glm_est, df$tmle_glm_ci_ic_lo, df$tmle_glm_ci_ic_hi,
                         df$tmle_glm_success, truth$RD_true, df$tmle_glm_se_ic,
                         label, "TMLE-GLM (IC)"),
      compute_metrics_v3(df$tmle_glm_est, df$tmle_glm_ci_mb_lo, df$tmle_glm_ci_mb_hi,
                         df$tmle_glm_success, truth$RD_true, df$tmle_glm_se_mb,
                         label, "TMLE-GLM (mult.boot)"),
      compute_metrics_v3(df$tmle_ml_est, df$tmle_ml_ci_ic_lo, df$tmle_ml_ci_ic_hi,
                         df$tmle_ml_success, truth$RD_true, df$tmle_ml_se_ic,
                         label, "TMLE-ML (IC)"),
      compute_metrics_v3(df$tmle_ml_est, df$tmle_ml_ci_mb_lo, df$tmle_ml_ci_mb_hi,
                         df$tmle_ml_success, truth$RD_true, df$tmle_ml_se_mb,
                         label, "TMLE-ML (mult.boot)"),
      compute_metrics_v3(df$cvtmle_est, df$cvtmle_ci_ic_lo, df$cvtmle_ci_ic_hi,
                         df$cvtmle_success, truth$RD_true, df$cvtmle_se_ic,
                         label, "CV-TMLE-ML (IC)"),
      compute_metrics_v3(df$cvtmle_est, df$cvtmle_ci_mb_lo, df$cvtmle_ci_mb_hi,
  ...
  for (fl in c("ok", "bad")) {
    ...
    if (nrow(sub) < 5) {
      ...
      gate[gate$overlap_flag == fl, c("tmle_ml_bias", "tmle_ml_cov_mb",
                                       "tmle_ml_rmse", "acceptable")] <- NA
      next
    }
    ...
    has_mb <- !is.na(ok_ml$tmle_ml_ci_mb_lo) & !is.na(ok_ml$tmle_ml_ci_mb_hi)
    cov_mb <- if (sum(has_mb) >= 5)
      mean((ok_ml$tmle_ml_ci_mb_lo[has_mb] <= truth$RD_true) &
           (truth$RD_true <= ok_ml$tmle_ml_ci_mb_hi[has_mb])) else NA

    # Reference RMSE from "ok" overlap (compute for ok stratum later; use overall as fallback)
    accept <- !is.na(cov_mb) && (abs(bias_ml) < 0.01) && (cov_mb >= 0.93)

    gate$n_reps[gate$overlap_flag == fl] <- nrow(sub)
    gate$tmle_ml_bias[gate$overlap_flag == fl] <- round(bias_ml, 5)
    gate$tmle_ml_cov_mb[gate$overlap_flag == fl] <- round(cov_mb, 3)
  ...
  tryCatch({
    ps_df <- data.frame(
      g_hat = unlist(lapply(seq_len(min(10, p$reps)), function(r) {
        sim <- sim_data_v3(p$n_main, seed = p$master_seed + r, p = p)
        estimate_ps_v3(sim$data, p)$g_raw
      })),
      flag = rep(
        unlist(lapply(seq_len(min(10, p$reps)), function(r) {
          sim <- sim_data_v3(p$n_main, seed = p$master_seed + r, p = p)
          ps <- estimate_ps_v3(sim$data, p)
          diag <- overlap_diagnostics_v3(ps$g_raw, ps$g_bounded, sim$data$A, p)
  ...
  tryCatch({
    bias_df <- summary_overall[summary_overall$subset %in% c("overlap_ok", "overlap_bad") &
                               !grepl("mult\\.boot", summary_overall$method), ]
    if (nrow(bias_df) > 0) {
  ...
  tryCatch({
    ...
    if (nrow(cov_df) > 0) {
      ...
      p_cov <- ggplot(cov_df, aes(x = method, y = coverage, fill = method)) +
        ...
        coord_flip() + ylim(0, 1) +
        labs(title = paste0("Coverage (", mode_name, " W4)"),
             x = NULL, y = "Coverage") +
        guides(fill = "none") +
        theme_minimal(base_size = 11)
      ggsave(file.path(fig_dir, "coverage_overall.png"), p_cov, width = 9, height = 6)
    }
...
message("=== Scenario 1 v3: Mixed Overlap, Nonlinear Q, CV-TMLE ===")
